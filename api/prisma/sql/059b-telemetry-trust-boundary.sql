-- ═══════════════════════════════════════════════════════════════════════
-- 059 — مرزِ اعتمادِ تله‌متری (فازِ ۲ · پروتکل §۱۴ و §۱۵)
--
-- ⚠️ نکته‌ی شماره‌گذاری (ثبت‌شده ۲۰۲۶-۰۸-۲۶): این فایل قبلاً «۰۴۶» بود و
-- هنگامِ merge به ۰۵۹ تغییرِ نام داد، ولی همین سرتیترش به‌روز نشد و تا امروز
-- «046» می‌گفت. ضمناً شماره‌ی ۰۵۹ با `059-restaurant-closures-fk-cascade.sql`
-- تصادم دارد.
--
-- چرا با وجودِ تصادم تغییرِ نام داده *نشد*: `apply-sql.sh` با glob روی
-- `*.sql` حلقه می‌زند، پس ترتیب قطعی است (`-r` پیش از `-t`)، هر دو فایل
-- idempotent‌اند و به جدول‌هایِ کاملاً جدا دست می‌زنند — یعنی تصادم اثرِ
-- اجرایی ندارد. در مقابل، تغییرِ نام دو سندِ ارجاع‌دهنده
-- (`docs/DATABASE.md`, `docs/architecture/AI-DATA-ELIGIBILITY.md`) را
-- می‌شکند. پس فقط سرتیترِ گمراه‌کننده اصلاح شد.
--
