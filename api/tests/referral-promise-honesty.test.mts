...
describe('A1-005 — وعده‌ی دعوت با مکانیزمش می‌خواند', () => {
  ...
  test('⚠️ خود-بازنشستگی: اگر پرداخت وصل شد، این گارد باید برداشته شود', () => {
    ...
    assert.deepEqual(
      ...
      'دوباره نشان بده، **دو ردیفِ `referral-*` را به manifestِ\n' +
      'tools/check-loyalty-constant-binding.mjs برگردان** (وگرنه ۵۰۰ به هیچ تعریفی بسته\n' +
      'نیست)، و **این فایل را حذف کن**. صداکننده(ها):\n  ' + callers.join('\n  '),
    );
  });

  test('⚠️ حذفِ ردیف‌های manifest تا وقتی مجاز است که وعده‌ای در کار نباشد', () => {
    // ⚠️ [۲۰۲۶-۰۹-۱۰] A1-005 دو لنگرِ `referral-*` را از گاردِ بایندینگ برداشت،
    // چون متنی که به آن‌ها لنگر می‌انداخت دیگر وجود نداشت. آن حذف **درست** بود
    // ولی یک بدهی ساخت: `POINTS.referralReward` هنوز تعریف است و حالا هیچ گاردی
    // آن را به UI نمی‌بندد. تا وقتی UI هیچ ادعایی ندارد این بی‌خطر است.
    //
    // این تست همان شرط را **اجرا** می‌کند: لحظه‌ای که پرداخت وصل شود، تستِ بالا
    // قرمز می‌شود و متنِ خطایش دقیقاً می‌گوید ردیف‌ها باید برگردند. اینجا فقط
    // مطمئن می‌شویم فرضِ زیربنایی هنوز برقرار است — که گارد واقعاً این دو را
    // ندارد و کسی بی‌صدا نصفه‌کاره برشان نگردانده.
    const guard = codeOnly(readFileSync(
      fileURLToPath(new URL('tools/check-loyalty-constant-binding.mjs', ROOT)), 'utf8'));
    const hasRows = /id:\s*'referral-(loyalty-card|sheet)'/.test(guard);

    if (completeReferralCallers().length > 0) {
      assert.ok(hasRows,
        'پرداخت وصل شده ⇒ وعده برمی‌گردد ⇒ دو ردیفِ `referral-*` باید در manifest باشند، ' +
        'وگرنه عددِ ۵۰۰ در UI به POINTS.referralReward بسته نیست');
      return;
    }
    assert.ok(!hasRows,
      'ردیفِ `referral-*` در manifest هست ولی متنِ وعده در UI نیست ⇒ گارد قرمزِ دائمی می‌دهد. ' +
      'یا لنگر باید به متنِ موجود به‌روز شود، یا ردیف برداشته شود — نیافتن هرگز «رد شد» نیست.');
  });
});
