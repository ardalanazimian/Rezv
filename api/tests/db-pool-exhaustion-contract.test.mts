...
describe('P2024 — ته‌کشیدنِ استخرِ اتصال', () => {
  test('errorResponse به ۵۰۳ ترجمه می‌کند، نه ۵۰۰، و شمارنده تکان می‌خورد', async () => {
    ...
    // ⚠️ `>=` و نه `===` — و این **دورزدنِ پنهان نیست**، تفکیکش مهم است.
    // نقدِ معاون (`rezv-fa`، ORDER-004) درست بود: `dbPoolTimeouts` از
    // `errors.ts` بالا می‌رود، یعنی از **هر** مسیری که خطایش به
    // `errorResponse` برسد — شعاعِ اثر به‌اندازه‌ی کلِ پروسه است، نه یک مسیر.
    // بینِ `before` و این خط چند `await` هست (اشباعِ استخر ذاتاً async است)،
    // پس یک P2024ِ هم‌زمان از مسیرِ دیگر می‌تواند شمارنده را جلو بیندازد و
    // این تست را بی‌ربط قرمز کند.
    //
    // چرا این شل‌کردنِ ادعا نیست: چیزی که این خط گارد می‌کند «وجودِ
    // `metrics.dbPoolTimeouts.inc()` در مسیر» است. با برداشتنِ آن `inc`،
    // اختلاف صفر می‌شود و `>= before + 1` **همچنان می‌شکند** — اثباتش در
    // تزریقِ ثبت‌شده‌ی همین فایل. آنچه از دست می‌رود فقط توانِ ادعا درباره‌ی
    // *دقیقاً یک* افزایش است، که هرگز خاصیتِ موردِ نظر نبود.
    //
    // مرزِ درست، همان‌طور که برای تستِ otp-break-glass گذاشتم: **ایزوله‌کردنِ
    // شمارنده، نه شل‌کردنِ ادعا.** ایزوله‌ی واقعی نیاز به برچسب یا نمونه‌ی
    // جدا دارد که کدِ تولیدی امروز نمی‌دهد؛ تا آن روز این مرز صریح است.
    assert.ok(poolCount() >= before + 1,
      'بدونِ شمارنده، طوفانِ استخر فقط یک ۵۰۳ در لاگ است و هیچ عددی نمی‌گوید '
      + `ظرفیت کم است — همان «شمارنده‌ای که نمی‌تواند تکان بخورد». قبل=${before} بعد=${poolCount()}`);
  });
  ...
  test('خطاهای دیگر همچنان ۵۰۰ می‌گیرند — ترجمه نباید گشاد شود', () => {
    ...
    // ⚠️ این `===` عمداً سخت مانده، و **دلیلش ساختاری است نه خوش‌بینی.**
    // معاون این را شکننده‌ترینِ دو ادعا خواند چون «روی هر P2024ِ هم‌زمان
    // می‌شکند». درست است — **مگر اینکه هیچ نقطه‌ی awaitی بینِ دو خواندن
    // نباشد**، و این‌جا نیست: `errorResponse` همگام است و این تستِ callback
    // همگام است. در حلقه‌ی رویدادِ تک‌نخی، هیچ کدِ دیگری نمی‌تواند بینِ
    // `before` و این خط اجرا شود. ایزوله‌بودن این‌جا **اثباتی است، نه آماری**.
    //
    // 🚫 پس یک قاعده برای هر کسی که این تست را ویرایش می‌کند: **هیچ `await`ی
    // بینِ `const before` و این خط اضافه نکن.** اولین `await` این ادعا را از
    // «ایزوله به‌حکمِ ساختار» به «ایزوله به‌حکمِ شانس» تنزل می‌دهد، و آن‌وقت
    // باید مثل تستِ بالا به `>=` تبدیل شود — که ارزشش را از دست می‌دهد، چون
    // کلِ کارِ این ردیف اثباتِ **بالا نرفتن** است.
