...
// (`table_id`ِ یکسان)، پس کانسترینتِ `no_table_overlap` به‌تنهایی همه‌ی ادعاهاشان
// را برآورده می‌کند — و کانسترینت یک قیدِ سطحِ دیتابیس است که مستقل از سطحِ
// ایزولاسیون شلیک می‌کند. اثباتِ اجراشده: جهشِ
// `Serializable → ReadCommitted` در `lib/reservations.ts:396` این فایل را
// **۶/۶ سبز، exit 0** نگه می‌دارد، در حالی که همان جهش
// `slot-lock-failopen-double-booking.test.mts` را قرمز می‌کند (۲ شکست، exit 1).
//
// یعنی تست درست است و **عنوانش** اشتباه بود. نیمه‌ی ایزولاسیون — تنها چیزی که
// از میزِ **ثانویه‌یِ** یک merge محافظت می‌کند، جایی که EXCLUDE ساختاراً کور است —
// اینجا سنجیده نمی‌شود و نباید هم بشود: پوششش در همان فایلِ slot-lock است و
// تکرارش دو منبعِ حقیقت برای یک خاصیت می‌سازد.
describe('C2 — قفلِ Redis کاملاً fail-open (شبیه‌سازیِ قطعیِ Redis) — کانسترینتِ EXCLUDE روی میزِ اصلی به‌تنهایی هنوز محافظت می‌کند', () => {
