...
describe('generateTimesFromHours', () => {
  ...
  test('شیفتِ بعد از نیمه‌شب فقط بخشِ همان روز را می‌دهد', () => {
    // ⚠️ **این تست در ۲۰۲۶-۰۸-۲۰ وارونه شد — و عمداً.**
    //
    // نسخه‌ی قبلی ادعا می‌کرد `['00:00','00:30','23:00','23:30']` درست است، و
    // خودش در توضیحش نوشته بود «همان رفتارِ فعلیِ generateTimesFromHours است،
    // نه چیزی که این تست باید تغییرش بدهد» — یعنی یک *مشاهده* را قفل کرده
    // بود، نه یک *قصد*. و همان مشاهده دقیقاً یک باگِ کاربر-رو بود:
    //
    // کلِ قراردادِ availability و رزرو date-keyed است — مصرف‌کننده
    // `zonedTimeToUtc(date, time)` صدا می‌زند. پس «۰۰:۳۰» به نیمه‌شبِ
    // *ابتدای* همان روز ترجمه می‌شد، نه به ۰۰:۳۰ِ شبِ بعد. برای کافه‌ای که
    // ۲۳:۰۰ باز می‌کند یعنی سانسی ۲۳ ساعت *پیش از* بازشدن — و چون مرتب‌سازی
    // رشته‌ای است، اولین گزینه‌ی مشتری هم بود.
    //
    // نتیجه‌ی واقعی: یا خطای «زمانِ گذشته» روی سانسی که خودِ اپ پیشنهاد داده،
    // یا رزروی ۲۴ ساعت زودتر از آنچه مهمان فکر می‌کرد.
    const oh = { '5': [['23:00', '01:00']] as [string, string][] };
    const times = generateTimesFromHours(oh, FRIDAY, 'Asia/Tehran', NO_CLOSURES)!;

    assert.deepEqual(times, ['23:00', '23:30'],
      'فقط سانس‌های داخلِ همان روزِ تقویمی');
    assert.ok(!times.includes('00:00') && !times.includes('00:30'),
      'سانسِ پس از نیمه‌شب به روزِ بعد تعلق دارد و زیرِ این قرارداد قابلِ بیان نیست');

    // ⚠️ محدودیتِ باقی‌مانده (مستند، نه پنهان): سانس‌های پس از نیمه‌شب فعلاً
    // اصلاً آنلاین رزرو نمی‌شوند. پشتیبانیِ درستشان یعنی حملِ «روز» در خودِ
    // قراردادِ سانس ("HH:mm" کافی نیست) که تغییری cross-cutting است.
    // پیشنهادنکردنِ یک سانس بهتر از پیشنهادِ سانسی است که زمانِ اشتباه رزرو کند.
  });
