...
/**
 * ⚠️ سازگاریِ موقتِ کلاینتِ کهنه — قابلِ حذف، ولی نه در همین تغییر.
 *
 * آخرین فراخوانِ شناخته‌شده (`markArrived` در
 * apps/business/js/reservations.js) در همین برنچ به `kind:'welcome'` مهاجرت
 * کرد، پس نقضِ رضایتِ بالا از مسیرِ پنل دیگر زنده نیست. نامِ مستعار عمداً
 * سرِ جایش می‌ماند: نسخه‌ی standalone/business.html و هر کلاینتِ کش‌شده‌ای که
 * هنوز کد قدیمی را اجرا می‌کند باید ایمن بماند؛ حذفش یک تصمیمِ جداست و باید
 * وقتی گرفته شود که لاگِ `log.warn` زیر برای مدتی خاموش مانده باشد.
 * همان ترکیبِ دقیق به‌عنوان نامِ مستعارِ `welcome` پذیرفته می‌شود:
