 ...
 *
 * قاعده‌ی مهاجرتِ ۰۷۲ این بود: **قدیمی‌ترین ثبت برنده است**؛ `id` فقط
 * شکنندهٔ تساوی است تا نتیجه قطعی بماند.
 *
 * ── ⚠️ اصلاحِ ۲۰۲۶-۰۹-۱۱: «قدیمی‌ترین» جلوی squattingِ شماره را نمی‌گرفت ──
 *
 * «قدیمی‌ترین» فقط ترتیب را **قطعی** کرد، نه **درست**. مهاجمی که شماره را
 * *پیش از* قربانی ثبت کند همچنان برنده است — و `POST /restaurant/staff`
 * هیچ اثباتِ مالکیتِ شماره نمی‌خواهد، پس ثبتِ زودتر هزینه‌ای ندارد.
 *
 * قاعده‌ی تازه: **ردیفِ `owner` بر هر ردیفِ دیگری مقدم است**، و بینِ بقیه
 * همان «قدیمی‌ترین ثبت» دست‌نخورده می‌ماند. دلیلش این است که `owner` تنها
 * نقشی است که با یک اثباتِ واقعی ساخته می‌شود: provisioning آن را می‌سازد و
 * یک `StaffInvite` پیامکی به **همان شماره** صادر می‌کند، و آن دعوت فقط با
 * اولین ورودِ موفقِ همان شماره `ACCEPTED` می‌شود (`acceptPendingInvites`).
 * به‌علاوه ایندکسِ یکتایِ جزئیِ ۰۷۹ (`staff (phone) WHERE role='owner'`)
 * تضمین می‌کند حداکثر **یک** ردیفِ owner با هر شماره وجود دارد، پس این
 * ترجیح خودش ابهامِ تازه‌ای نمی‌سازد.
 *
 * ⚠️ باقی‌مانده‌ی صادقانه (پنهان نشده): این **کاملاً** نمی‌بنددش. اگر شماره‌ای
 * هیچ ردیفِ `owner` نداشته باشد، یک ردیفِ `staff`ِ اثبات‌نشده همچنان ورودِ
 * OTPِ آن شماره را می‌گیرد — یعنی کارمندِ واقعیِ رستورانِ X که هنوز ownerی
 * با شماره‌اش وجود ندارد، می‌تواند توسطِ تنانتِ دیگری ربوده شود. بستنِ کاملش
 * یک سیگنالِ «این شماره اثبات شد» به‌ازای هر ردیف می‌خواهد، و امروز چنین
 * چیزی مشتق‌شدنی نیست: `POST /restaurant/staff` اصلاً `StaffInvite` نمی‌سازد
 * (تنها سازنده‌اش `provisioning.ts` است)، پس محدودکردنِ جست‌وجو به «دعوتِ
 * پذیرفته‌شده» ورودِ **همه‌ی** کارکنانِ عادی را می‌کشت. رفعِ کامل = فلوی
 * دعوت برای staff هم، که مهاجرت/route می‌خواهد و عمداً در این تغییر نیست.
 *
 ...
 */
export async function findStaffForLogin(normalizedPhone: string) {
  // ⚠️ دو کوئریِ جدا و نه یک `orderBy: [{ role: 'asc' }, …]`: ترتیبِ enumِ
  // `staff_role` در Postgres `owner < manager < staff` است، پس مرتب‌سازیِ
  // خامِ نقش، `manager` را هم بر `staff` مقدم می‌کرد — ترجیحی که هیچ اثباتی
  // پشتش نیست (هر دو از همان مسیرِ بی‌اثباتِ `POST /restaurant/staff` می‌آیند).
  // این‌طور فقط همان تمایزی اعمال می‌شود که واقعاً توجیه دارد.
