...
export const RULES = {
  ...
  // یک رستوران همه پشتِ یک NATِ وای‌فای‌اند، یعنی یک IP. بدتر: بن‌کنندهٔ
  // خودکار با ۱۰ بار خوردنِ ریت‌لیمیت در ۵ دقیقه همان IP را **یک ساعت**
  // کامل می‌بندد، پس شلوغیِ یک شبِ جمعه می‌توانست کلِ وای‌فای رستوران را
  // قطع کند. ۶۰ ظرفیتِ *به‌ازای هر چک‌این* را به همان جای قبل برمی‌گرداند
