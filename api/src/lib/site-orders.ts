...
export function slugSeed(businessName: string, fallback: string): string {
  const slug = businessName
    ...
    // ⚠️ اعراب (D-31 · m-24): کلاسِ پایین کلِ بلاکِ ؀-ۿ را نگه می‌دارد و اعراب هم
    // همان‌جاست. نتیجه‌اش slugی با کسره‌ی **نامرئی** بود — کاربری که نام را
    // می‌خواند و تایپ می‌کند به ۴۰۴ می‌رسید، و NFC هم دو املا را یکی نمی‌کند
    // (علامتِ ترکیبیِ مستقل فرمِ ترکیب‌شده ندارد). حروفِ فارسی می‌مانند؛ فقط اعراب می‌رود.
    .replace(/[\u064B-\u0652\u0670]/g, '')
