...
/**
 * ماه و روزِ **شمسیِ** یک لحظه‌ی UTC در تهران، به‌صورتِ عددِ ۱..۱۲ و ۱..۳۱.
 *
 * ⚠️ این تابع دیگر در مقایسه‌ی تولد استفاده **نمی‌شود** — رجوع کن به بلوکِ
 * «یک قرارداد» داخلِ grantBirthdayRewards. ستونِ users.birth_date در همه‌ی
 * نقاطِ نوشتن میلادی است، و cron هم میلادی مقایسه می‌کند.
 *
 * عمداً export مانده چون مرزِ **نمایش** به آن نیاز دارد: پنلِ رستوران تاریخ‌ها
 * را شمسی نشان می‌دهد (`api/src/app/api/v1/restaurant/members/route.ts`). حذفش
 * یعنی همان تبدیل جای دیگری دوباره نوشته شود — دقیقاً همان تکثیری که این
 * زنجیره‌ی باگ را ساخت.
 *
...
export async function grantBirthdayRewards(): Promise<{ birthday: number; anniversary: number }> {
  ...
  // ═════ یک قرارداد، از نوشتن تا نمایش (رفعِ ممیزیِ ۲۰۲۶-۰۹-۱۱) ═════
  //
  // `users.birth_date` همه‌جا **میلادی** نوشته می‌شود. این ادعا از روی هر سه
  // نقطه‌ی نوشتن ردیابی شده، نه فرض:
  //  ۱. واک‌ینِ پنل: سلکت ماهِ **شمسی** می‌دهد، ولی خودِ فرم پیش از ارسال به
  //     میلادی تبدیل می‌کند (`apps/business/js/reservations.js`،
  //     jalaliMdToGregMd) و `createWalkinTx` همان عددِ میلادی را در
  //     `Date.UTC(1990, m-1, d)` می‌نشاند (`lib/reservations.ts`).
  //  ۲. `POST /restaurant/members` ورودی‌اش را صریح میلادی مستند می‌کند
  //     (`lib/club-enroll.ts`).
  //  ۳. `PATCH /me` یک تاریخِ ISOِ واقعی می‌نویسد (`app/api/v1/me/route.ts`).
  //
  // پس مقایسه هم باید میلادی باشد. `lib/automation.ts` (کمپینِ تولدِ رستوران)
  // از اول میلادی مقایسه می‌کرد و درست بود؛ فقط همین cron نبود.
  //
  // ⚠️ باگی که این خط رفع می‌کند: نسخه‌ی قبلی `jalaliMonthDayToday(today)` را
  // با `EXTRACT(MONTH/DAY FROM birth_date)` مقایسه می‌کرد — دو مقیاسِ متفاوت.
  // آن مقایسه برای دوره‌ای درست بود که واک‌ین عددِ **شمسی** را خام می‌فرستاد؛
  // بعد از اینکه تبدیل به مرزِ فرم منتقل شد، این طرف به‌روز نشد و هدیه‌ی ۱۰۰۰
  // امتیازی برای تقریباً همه در روزِ اشتباه شلیک می‌شد. هیچ‌کس متوجه نمی‌شد
  // چون هدیه واقعاً ارسال می‌شد — فقط روزش غلط بود.
  //
  // تهران، نه UTC: cron ممکن است ساعتِ محلیِ سرور را نداشته باشد و نزدیکِ
  // نیمه‌شبِ تهران (UTC+03:30) روزِ UTC یک روز عقب است. `dateInTz` همان
  // کمکیِ کانونیِ مخزن است (`lib/hours.ts`) که availability و lifecycle هم
  // از آن استفاده می‌کنند — کتابخانه‌ی تازه‌ای اضافه نشده.
  //
  // نمایش همچنان شمسی است، ولی فقط در مرزِ پنل (route، با
  // `jalaliMonthDayToday` که به همین دلیل export مانده) — نه در ذخیره و نه
  // در این مقایسه.
  const { m: mm, day: dd } = dateInTz(today, 'Asia/Tehran');
