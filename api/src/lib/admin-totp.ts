 ...
 *
 * ⚠️ **تصحیحِ ادعا (دستورِ ۰۴۵، ۲۰۲۶-۰۹-۱۰).** این‌جا می‌گفت «هیچ‌وقت `throw`
 * نمی‌کند». **غلط بود:** `redis.set` در خطِ ضدِ replay wrap نشده و در قطعیِ
 * Redis پرتاب می‌کند. ادعا از کدش عمر بیشتری کرده بود — همان کلاسی که این
 * مخزن قبلاً بابتش هزینه داده (کامنتِ «`npm ci` قفل را رعایت می‌کند» در
 * `ci.yml` که خطِ زیرش خنثی‌اش می‌کرد).
 *
 * آنچه **واقعاً** درست است و چرا نگهش داشتیم:
 *
 * ۱. **مسیرِ راز**: هر شکستِ مربوط به پیکربندی یا خودِ کد، **مقدار**
 *    برمی‌گرداند (`not_configured` / `invalid`)، نه استثنا. پس اوراکلی که
 *    این کامنت نگرانش بود — ۵۰۰ در برابرِ ۴۰۱ برای «این حساب راز دارد؟» —
 *    بسته می‌ماند. قاعده‌ی `verifyPassword` (`password.ts:77`) رعایت شده.
 *
 * ۲. **تنها استثنای ممکن** از `redis.set`ِ ضدِ replay می‌آید، و رسیدن به آن
 *    خط نیازمندِ یک کدِ **معتبر** است (`delta !== null`). یعنی مهاجمی که
 *    می‌تواند این ۵۰۰ را بسازد، از قبل کدِ درست را دارد؛ اوراکل چیزی به او
 *    نمی‌گوید که نداند.
 *
 * ۳. اثرش **fail-closed** است: قطعیِ Redis ورود را رد می‌کند، نه اینکه
 *    ضدِ replay را بی‌صدا خاموش کند. و ۵۰۰ صادق‌تر از ۴۰۱ است — «زیرساختِ ما
 *    خراب شد» در برابرِ «کدِ تو غلط بود». پس عمداً wrap **نشد**.
 */
