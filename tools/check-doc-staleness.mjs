...
const HISTORY = [
  ...
  // ⚠️ رونوشتِ حمله‌هایِ Red Team: نامِ متغیرهایِ probe عمداً ساختگی‌اند، و
  // تغییرِ نامشان برایِ سبزکردنِ این گیت یعنی **ویرایشِ شواهد** — دقیقاً همان
  // چیزی که این ابزار برایِ گرفتنش هست. افزوده توسطِ `rezv-c7` (۲۰۲۶-۰۹-۰۹)
  // که خودش هم اعلام کرد گاردِ نشستِ دیگری را ویرایش می‌کند، به‌جای اینکه
  // ضمنی بگذاردش.
  //
  // ⚠️ باقی‌مانده‌ی شناخته‌شده، برایِ نفرِ بعد: این معافیت **کلِ پوشه** است، و
  // آن پوشه فقط رونوشتِ حمله ندارد — فایل‌هایِ `RETEST-*` ادعایِ **زنده**
  // درباره‌یِ محصول می‌کنند. پس یک ادعایِ پیکربندیِ کهنه در یک RETEST دیگر
  // این‌جا قرمز نمی‌شود. عمدی و پذیرفته‌شده است (سه پوشه‌ی هم‌جنسِ بالا همین
  // رفتار را دارند) ولی **ثبت‌شده، نه فراموش‌شده**: اگر روزی دقتِ بیشتری لازم
  // شد، رفعش تفکیکِ رونوشت‌ها از RETESTهاست، نه برداشتنِ این خط.
  'docs/audit/redteam/',
];
