...
//   review ۲۰ → ۱۷: ۳۶ سینک در همان کامیت بی‌لنگر شده بودند؛ امروز یکی‌یکی با
//     عبارتِ کامل بازبینی و در MANUAL_REVIEW_OVERRIDES ثبت شدند. ۱۷ تای
//     باقی‌مانده عمداً بازبینی‌نشده‌اند و در شمارش می‌مانند — دلیلشان بالای
//     همان دسته نوشته شده.
// هیچ‌کدام بالا نرفت، و هیچ سینکی برای سبز شدن بی‌بازبینی رها نشد.
const UNSAFE_BASELINE = 42;
const REVIEW_BASELINE = 17;
...
const MANUAL_REVIEW_OVERRIDES = new Map([
  ...
  ['apps/business/js/overview.js#3b20e44a5673',
   'داشبورد. `liveStatusBadge()` (overview.js:488) و `dashboardUsingDemoData()` (:483) فقط '
   + 'ثابت و icon() برمی‌گردانند؛ بقیه fa() روی KPIهای عددی است.'],
