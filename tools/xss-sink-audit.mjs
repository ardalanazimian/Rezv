...
// ۲۰۲۶-۰۹-۱۱ — هر دو عدد **پایین** آمدند تا پیشرفت قفل شود (خودِ ابزار خواست):
//   unsafe ۶۵ → ۴۲: این کاهش از `63447e2` آمد نه از این نشست — استخراجِ درست،
//     ۲۲ سینک را از unsafe بیرون برد و عدد از آن روز نامقفل مانده بود.
//   review ۲۰ → ۱۸: ۳۵ سینک در همان کامیت بی‌لنگر شده بودند؛ یکی‌یکی بازبینی و
//     در MANUAL_REVIEW_OVERRIDES ثبت شدند. ۱۸ تای باقی‌مانده عمداً بازبینی‌نشده‌اند
//     و در شمارش می‌مانند — دلیلشان بالای همان دسته نوشته شده.
//     ⚠️ عدد اولش ۱۷ بود: یک override را **پس گرفتم** (داشبوردِ
//     `overview.js:172`) چون آن سینک روی سقفِ ۴۰۰۰ کاراکتری بریده می‌شود و
//     ادامه‌اش دو مقدارِ خام داشت. توضیحِ کامل کنارِ جای خالی‌اش در جدول.
// هیچ‌کدام از خطِ پایه‌ی امروزِ main (۶۵/۲۰) بالاتر نرفت، و هیچ سینکی برای سبز
// شدن بی‌بازبینی رها نشد.
const UNSAFE_BASELINE = 42;
const REVIEW_BASELINE = 18;
...
const MANUAL_REVIEW_OVERRIDES = new Map([
  ...
  // ═══════════════════════════════════════════════════════════════════
  //  دسته‌ی ۲۰۲۶-۰۹-۱۱ — رد تیم (`rezv-8e [6b665b]`)
  //
  //  چرا این دسته لازم شد: `63447e2` استخراج‌کننده را درست کرد و عبارتِ
  //  ۹۶ سینک **بلندتر** شد. چون کلیدِ override هشِ همان عبارت است، ۷۷ از
  //  ۸۷ کلید بی‌لنگر شدند و شمارشِ review از ۲۰ به ۵۳ رفت. آن کامیت خودش
  //  نوشت که این‌ها آسیب‌پذیریِ تازه نیستند؛ سینک‌هایی‌اند که حکمِ قبلی‌شان
  //  روی **خواندنِ ناقص** صادر شده بود. پس هر ردیفِ زیر با عبارتِ کاملِ
  //  امروز دوباره خوانده شد، نه با اعتماد به یادداشتِ قبلی.
  //
  //  روشِ بازبینی (قابلِ تکرار): هر `${…}`ِ سینک به‌صورت بازگشتی — از جمله
  //  داخلِ templateهای تودرتو — تا برگ باز شد، و فقط برگ‌هایی که نه esc()/
  //  jsq() دارند، نه icon()، نه literal/حساب/اندیسِ حلقه‌اند، دستی خوانده
  //  شدند. هر یادداشت مسیرِ همان شاهد را می‌گوید.
  //
  //  ⚠️ عمداً بازبینی‌نشده ماند (۱۷ سینک، همه در شمارشِ review می‌مانند):
  //   • سینک‌های عمومیِ `openModal(html)`/`openSheet(html)` — ایمنی‌شان به
  //     **همه‌ی** فراخوان‌ها بستگی دارد، نه به خودِ سینک؛ این یک بازبینیِ
  //     جداست و ادعای زودهنگام درباره‌اش بدتر از قرمز ماندن است.
  //   • اپِ مشتری — چند سینکش به `faTime`/`faNum`/`fmtFa` روی رشته‌ی سرور
  //     تکیه دارد و آن‌ها **escaper نیستند** (گزارشِ جداگانه‌ی همین نشست:
  //     `fa()` و پنج هم‌رفتارش رشته را دست‌نخورده رد می‌کنند، دقیقاً همان
  //     دلیلی که `faNum` با آن از فهرستِ مورد‌اعتماد کنار گذاشته شده بود).
  //     تا آن رفع نشود، سبز کردنشان یعنی ثبتِ ادعایی که پشتش نیست.
  //   • `company/js/sales.js:125` — `renderSalesOrders()`/`renderSalesInquiries()`
  //     را نخواندم؛ بازبینی‌نشده یعنی بازبینی‌نشده.
  // ═══════════════════════════════════════════════════════════════════
  ['apps/business/js/crm.js#c8e90d5259c3',
   'گالریِ عکس. تنها درجِ خامش `${g.emoji}` است و آن همیشه تهی است: ردیف‌های گالری در '
   + 'crm.js:79-80 از API با `emoji:\'\'` ساخته می‌شوند. url/label/caption همگی از esc() رد '
   + 'می‌شوند؛ بقیه fa() روی شمارنده، ترنریِ رشته‌ی ثابت، و اندیسِ حلقه است.'],
  ['apps/business/js/crm.js#b83c60fdb23d',
   'کارتِ نظرهای مهمان. متنِ نوشته‌ی مهمان و نامش در همین قالب esc() دارند؛ دو درجِ بازمانده '
   + 'یکی درصدِ محاسباتی (`d.count/totalCount*100`) و دیگری '
   + '`Array.from({length:5},…icon(\'star\'))` است — هیچ‌کدام متن حمل نمی‌کنند.'],
  ['apps/business/js/crm.js#4f30ce2c522c',
   'کارتِ سگمنت‌ها. همه‌ی درج‌ها fa() روی شمارنده/درصدِ محاسبه‌شده‌اند (`segTotal`، '
   + '`s.count`، `p`) به‌علاوه icon()؛ نامِ سگمنت از نگاشتِ ثابتِ درون‌فایل می‌آید.'],
  ['apps/business/js/crm.js#2a43296c52f5',
   'هوشِ مشتری. fa()/faOrDash() روی شمارنده‌ها، و `cards.slice(0,3).map(...)` که متنِ '
   + 'کارت‌ها را esc() می‌کند؛ شدت (`c.severity`) فقط با رشته‌های ثابت مقایسه می‌شود و به '
   + 'کلاسِ CSS تبدیل می‌شود.'],
  ['apps/business/js/crm.js#3865b4d927ed',
   'فهرستِ پروفایلِ مشتری‌ها. `iqClr`/`iqFa`/`urgClr` نگاشت‌های ثابتِ درون‌فایل‌اند '
   + '(crm.js:735-736)، `urg` از مقایسه‌ی عددیِ churn ساخته می‌شود، و نام/تلفنِ مشتری در همین '
   + 'قالب esc()/jsq() دارند. بقیه fa() روی امتیازهای عددی است.'],
  ['apps/business/js/crm.js#4ada2ae211cc',
  ...
  ['apps/business/js/marketing.js#d96ef285af55',
   ...
   + 'همان نگاشتِ ثابت می‌آیند، `c.id` یک UUID در مقدارِ option است، و نامِ قانون esc() دارد.'],
  ['apps/business/js/marketing.js#379b6b144252',
   'تحلیل‌ها. `trendChip` (marketing.js:384)، `dataSourceNote()` (data.js:653) و '
   + '`rAnalyticsTail()` (:430) رشته‌های ساخته‌شده از icon()/ثابت‌اند؛ `d`/`l`/`p`/`v` عناصرِ '
   + 'آرایه‌های عددیِ محلیِ `A.*`‌اند و بقیه محاسبه‌ی درصد است.'],
  ['apps/business/js/menu.js#2ac12446a42a',
   'برندینگِ منو. هیچ درجِ اثبات‌نشده‌ای ندارد: رنگ و نام از esc() رد می‌شوند و بقیه ثابت '
   + 'است.'],
  ['apps/business/js/menu.js#7e2aa5f93d40',
   'گروه‌های افزودنی. نامِ گروه/آیتم esc() و شناسه‌ها jsq() دارند؛ `g.id` یک UUIDِ Prisma '
   + 'است، و `fa(g.min_select)`/`fa(g.max_select)`/`fnMoney(o.price_delta_toman)` عددی‌اند.'],
  // ⚠️ `apps/business/js/overview.js#3b20e44a5673` (داشبورد، خطِ ۱۷۲) عمداً این‌جا
  //    **نیست** — نسخه‌ی اولِ همین دسته داشتش و پس‌گرفتمش.
  //
  //    چرا: عبارتِ آن سینک دقیقاً روی سقفِ ۴۰۰۰ کاراکتریِ `grabExpression`
  //    می‌ایستد (۴۰۰۰ از ۴۰۰۰)، پس آنچه من و طبقه‌بند دیدیم **تکه‌ی اولِ**
  //    قالب بود. وقتی ادامه‌اش را مستقیم از فایل خواندم، دو مقدارِ خام پیدا شد
  //    که هیچ گاردی نمی‌بیندشان چون پشتِ یک فراخوانِ تابع‌اند:
  //      • `renderDashResvRows()` (:103) → `${initials(r.name)}` یعنی دو
  //        کاراکترِ اولِ نامِ مهمان، بدونِ esc — و `${r.t}` خام.
  //      • همان تابع نامِ کامل را درست esc می‌کند (`${esc(r.name)}`)، پس
  //        این یک قلمِ جاافتاده است، نه یک الگو.
  //    با دو کاراکتر نمی‌شود تگ ساخت و بست، پس این را «آسیب‌پذیریِ اجراپذیر»
  //    نمی‌نامم؛ ولی داده‌ی مهمان است که escape نشده و ادعای «همه‌چیز امن
  //    است» را باطل می‌کند. یک ردیفِ جدا می‌خواهد، نه یک override.
  //
  //    درسِ عمومی‌اش از خودِ مورد بزرگ‌تر است: **سینکی که روی سقف بریده شده،
  //    نمی‌تواند subject یک override باشد** — چون یادداشت درباره‌ی چیزی حکم
  //    می‌دهد که نویسنده‌اش کاملش را ندیده. پنج سینک امروز روی سقف‌اند
  //    (`detail.js:158`، `loyalty.js:42`، `overview.js:172`،
  //    `intelligence.js:353`، `company/overview.js:37`). تنها موردی که با
  //    وجودِ سقف override گرفت `company/overview.js#14106012de95` است و آن‌جا
  //    ادامه‌ی قالب را **دستی خواندم** (همه‌اش `esc()`/`fa()`/`icon()`/`jsq()`)
  //    و در یادداشتش نوشتم. `assertCallSinksFullyCaptured` این کلاس را نمی‌گیرد،
  //    چون فقط سینک‌های فراخوان‌محور را می‌سنجد نه انتساب‌محور.
  ['apps/business/js/overview.js#516bee785341',
   'نقشه‌ی حرارتی. `html` در همان تابع (overview.js:326-335) از آرایه‌های محلیِ '
   + '`days`/`slots` و `fa(v)` ساخته می‌شود — هیچ داده‌ی API‌ای در آن نیست.'],
  ['apps/business/js/overview.js#9c789e95502a',
   'مشتریانِ برتر. نام از esc() و از jsq() در onclick رد می‌شود، `c.seg===\'vip\'?…:\'\'` ترنریِ '
   + 'ثابت است و `fa(c.visits)` عددی.'],
  ['apps/business/js/overview.js#6b996ae43a03',
   'یادداشت‌های کارکنان. متنِ یادداشت `${esc(n.txt)}` است و `n.pinned?…:\'\'` فقط یک '
   + 'attributeِ styleِ ثابت می‌گذارد.'],
  ['apps/business/js/reservations.js#d72d35d36a3e',
   'صفحه‌ی رزروها. `TONIGHT_FILTERS` آرایه‌ی ثابتِ درون‌فایل است (reservations.js:8) پس '
   + '`f.l`/`f.v` رشته‌ی ثابت‌اند، onclick از jsq() رد می‌شود و بقیه icon() است.'],
  ['apps/business/js/reservations.js#371f2f25b9a3',
   'سرخطِ شمارشِ رزرو. `fa(list.length)`/`fa(seated)` عددی‌اند و عبارتِ جست‌وجوی کاربر '
   + '`${esc(resQuery)}` است.'],
  ['apps/business/js/staff-system.js#8cc487919cb2',
   'مرکزِ اعلان. `NOTIFS` آرایه‌ی محلی است (staff-system.js:172-)، `n.unread?…:\'\'` ترنریِ '
   + 'ثابت، و `n.ic`/`n.emoji` از esc()/icon() رد می‌شوند.'],
  ['apps/business/js/staff-system.js#73d998a96592',
   'قیمت‌گذاری هوشمند. `sugCard` (staff-system.js:331) رشته‌های آمده از API را عمداً esc() '
   + 'می‌کند (توضیحش همان‌جا ثبت است)، `dataSourceNote()` ثابت است و `toman(r.min_toman)` '
   + 'عددی.'],
  ['apps/business/js/waitlist.js#ba4edac61dac',
   'پلانِ سالن. نام/برچسبِ میز از esc() رد می‌شوند و بقیه شمارنده‌ی اشغال و کلاسِ CSSِ ثابت '
   + 'است.'],
  ['apps/business/js/waitlist.js#bf6fc3407fef',
   'قالبِ چاپِ QRِ میز. `_tableQrSvg` خروجیِ `QRCode.toString(url,{type:\'svg\'})` از '
   + 'کتابخانه‌ی qrcode است که از endpointِ احرازشده‌ی `GET restaurant/tables/:id/qr` می‌آید '
   + '(بازبینیِ ۲۰۲۶-۰۹-۰۵ که با تغییرِ هویتِ سینک بی‌لنگر شده بود؛ دوباره خوانده و تأیید شد). '
   + 'برچسبِ کنارش esc() دارد.'],
  ['apps/company/js/badges.js#951c06698587',
   'نشان‌های پلتفرم. `b.isActive?\'\':\'…\'` ترنریِ ثابت است؛ نام و توضیحِ نشان esc() دارند.'],
  ['apps/company/js/hours.js#a7cfa8b75684',
   'صفِ تأییدِ ساعتِ کاری. شاخه‌های loading/demo رشته‌ی ثابت‌اند و '
   + '`renderHoursChangeQueue()` (hours.js:153) داده‌اش را esc() می‌کند؛ `fa(HCHANGE_BADGE)` '
   + 'عددی است.'],
  ['apps/company/js/intelligence.js#2b1af64528a8',
   'KPIهای هوشِ پلتفرم. همه‌ی درج‌ها fa() روی شمارنده‌اند و نامِ رستوران در نگاشت‌های تودرتو '
   + 'esc() دارد.'],
  ['apps/company/js/intelligence.js#4a5edea6ba0b',
   'اشتراک‌ها. `PLAN_LABEL`/`STATUS_LABEL` نگاشت‌های ثابتِ data.js:23-24‌اند، `badgeCls` از '
   + 'مقایسه با رشته‌ی ثابت ساخته می‌شود، و بقیه fa() روی عدد است.'],
  ['apps/company/js/intelligence.js#3516a5f15892',
   'نوارِ وضعیتِ رستوران. `STATUS_LABEL[r.status]||\'\'` — fallbackش رشته‌ی تهی است، نه کلیدِ '
   + 'خام — و درجِ دیگر ترنریِ ثابت است.'],
  ['apps/company/js/intelligence.js#efc98279eb9f',
   'سلامتِ سیستم. `healthMeta` جدولِ ثابتِ درون‌خطی است (intelligence.js:233) و بقیه fa() '
   + 'روی شمارنده‌های صف/وبهوک است.'],
  ['apps/company/js/missions.js#65d2c43d791b',
   'ماموریت‌های پلتفرم. عنوان و شناسه esc() دارند؛ `m.status` فقط با رشته‌های ثابت مقایسه '
   + 'می‌شود و در یک `data-status` می‌نشیند، و بقیه fa() روی پاداش/هدفِ عددی است.'],
  ['apps/company/js/overview.js#14106012de95',
   'نمای کلیِ شرکت. `r.logo`/`r.grad` هرگز از API نمی‌آیند — `mapAdminRestaurant` '
   + '(company/js/api.js:137-138) آن‌ها را فقط از ردیفِ نمونه‌ی محلی پر می‌کند و در همان‌جا '
   + '«تزئینی» علامت خورده‌اند. `healthMeta`/`clvLine` محلی‌اند، نامِ رستوران esc() دارد و '
   + 'بقیه fa()/محاسبه‌ی درصد است. ⚠️ این سینک روی سقفِ ۴۰۰۰ کاراکتری می‌ایستد، پس '
   + '۷۸۱ کاراکترِ آخرش را طبقه‌بند اصلاً ندیده و من **مستقیم از فایل** خواندمش: '
   + 'ردیف‌های هشدارِ اشتراک‌اند با `esc(r.name)`، `fa(r.daysLeft)`، `icon()` و '
   + '`jsq(r.id)` در onclick — هیچ درجِ خامی ندارد.'],
  ['apps/company/js/overview.js#d8448053170b',
   'فهرستِ رستوران‌ها. همان نگاشتِ api.js:137-138 برای logo/grad؛ `statusText` از '
   + '`STATUS_LABEL` و شاخه‌های ثابت ساخته می‌شود و نامِ رستوران esc() دارد.'],
  ['apps/company/js/photos.js#a185d3b8fd37',
   'صفِ بازبینیِ عکس. `renderPhotoQueue()` (photos.js:167) نشانی و کپشن را esc() می‌کند، '
   + '`PHOTO_CATEGORY` نگاشتِ ثابت است و شاخه‌های loading/demo ثابت‌اند.'],
  ['apps/company/js/restaurant.js#bbebbb1909c7',
   'جزئیاتِ رستوران. logo/grad همین‌جا هم از esc() رد می‌شوند و `PLAN_LABEL` نگاشتِ ثابت '
   + 'است؛ بقیه fa() روی عدد.'],
  ['apps/company/js/restaurant.js#539e42b38ef1',
   'باشگاهِ رستوران. `PLAN_LABEL[p]||p` روی کلیدهای پلن (`apiR.plan||\'free\'`) و بقیه fa() '
   + 'روی شمارنده‌هاست.'],
]);

function scanFile(absPath, relPath) {
  // ⚠️ نرمال‌سازیِ پایانِ خط — بدونش **هویتِ سینک** به کپیِ کاریِ هر نفر گره
  //    می‌خورد، نه به کدی که کامیت شده (۲۰۲۶-۰۹-۱۱).
  //
  //  این اسکنر بایتِ خام را از دیسک می‌خواند و `sink_hash` را از
  //  `expr + sourceLine` می‌سازد. اگر کپیِ کاریِ کسی CRLF باشد، هر دو عوض
  //  می‌شوند، پس کلیدهای override بی‌لنگر می‌شوند و `--check` می‌گوید
  //  «آرتیفکت کهنه است» — و بازتولیدِ دوباره درستش نمی‌کند، فقط یک آرتیفکتِ
  //  CRLF-محور کامیت می‌کند که برای بقیه خراب است.
  //
  //  بازتولیدِ واقعی، نه فرضی: `apps/company/js/intelligence.js:353` با LF
  //  هشِ `3af3d99dd758` می‌دهد و همان فایل با CRLF دقیقاً `b41c49e82423` —
  //  همان عددی که نشستِ دیگر روی درختِ خودش دید. و با ۸۵ فایلِ تبدیل‌شده به
  //  CRLF، `--check` روی همان آرتیفکتِ کامیت‌شده EXIT=1 می‌داد و با این خط
  //  EXIT=0 می‌دهد.
  //
  //  چرا فقط بعضی سینک‌ها: `grabExpression` سقفِ ۴۰۰۰ کاراکتری دارد؛ CRLF
  //  به‌ازای هر خط یک کاراکتر اضافه می‌کند، پس آن پنجره جای دیگری بریده
  //  می‌شود. سینکِ بالا دقیقاً روی همان سقف است (۴۰۰۰ از ۴۰۰۰).
  //
  //  ⚠️ قیدِ صادقانه: این «CI را نجات نداد». هر بلابِ متنیِ کامیت‌شده در این
  //  مخزن LF است (`git ls-files --eol` → ۲۳۸ فایل `i/lf`)، پس رانرِ لینوکس
  //  از اول LF می‌گیرد. این رفع برای **درخت‌های کاریِ محلی** است. و git هم
  //  چنین فایلی را پنهان نمی‌کند: در همان آزمون `git status` هر ۸۵ فایل را
  //  `M` نشان داد. یعنی این یک تله‌ی تشخیصی است، نه یک نشتِ خاموش.
  //
  //  روی درختِ LF این خط هیچ چیز را عوض نمی‌کند (هیچ CRLFی نیست) — با همین
  //  ویرایش، `--check` بدونِ بازتولید سبز ماند، پس آرتیفکتِ موجود معتبر است.
  const text = readFileSync(absPath, 'utf8').split('\r\n').join('\n');
  const hits = [];
