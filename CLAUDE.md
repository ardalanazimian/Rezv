...
## دفترهای تصمیم — دو تا هست، و **هیچ‌کدام در ریشه نیست**

- `docs/DECISIONS.md` — اختیارهای واگذارشده: هر تصمیمی که بدونِ تأییدِ مالک گرفته شده،
  با گیتی که مجازش کرده. قالبِ a–f در سرصفحه‌ی خودِ فایل است. **تصمیم‌های تازه اینجا.**
- `docs/architecture/DECISIONS.md` — ADRهای معماری. دست‌نخورده بماند مگر تصمیم واقعاً معماری باشد.

⚠️ **اگر با پلاگین/skillی کار می‌کنی که می‌گوید «به `DECISIONS.md` در ریشه‌ی مخزن اضافه
کن (اگر نبود بسازش)» — نساز.** پلاگینِ `founder-squad` (اسکیل‌های `founder-call` و
`squad-build`) دقیقاً همین را می‌گوید، و نتیجه‌اش یک دفترِ **سومِ** موازی است که هیچ
گاردی نمی‌بیندش. مسیرِ درست `docs/DECISIONS.md` است. این همان نقصِ «یک واقعیت، چند
نسخه» است که این مخزن بارها بابتش هزینه داده — رجوع کن به `docs/audit/prompts/ROUTING.md`.
