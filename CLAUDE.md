...
> **وضعیتِ فعلیِ CI (۲۰۲۶-۰۸-۲۶، پس از merge):** ۱۰ جاب — `build`, `test`,
> `schema-drift`, `security`, `e2e`, `design-system`, **`standalone`**, `seo`,
> `landing`, `base-freshness`. همه باید سبز باشند (merge-on-green).
>
> ⚠️ `standalone` در ۲۰۲۶-۰۸-۲۶ اضافه شد. تا آن روز، گیتِ §۴b («تازگیِ بسته‌ی
> آفلاین») **فقط در همین فایل** وجود داشت و هیچ جابی اجرایش نمی‌کرد — یعنی
> تنها ضامنش این بود که یک آدم یادش بماند، و دقیقاً همان‌طور هم شکست
> (بسته ۵ روز کهنه ماند و یک P0 تحویل می‌داد). گیتی که فقط در markdown
> زندگی می‌کند، گیت نیست.
