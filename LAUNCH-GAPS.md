...
> ⚠️ **اصلاح‌شده (۲۰۲۶-۰۸-۲۰) — این سرخط با جدولِ زیرِ خودش در تناقض بود.**
> می‌گفت «۳ موردِ باز باقی مانده: ۳، ۵، ۷»، در حالی که هر سه در همان جدول ✅
> علامت خورده‌اند. یک سندِ «شکافِ لانچ» که کارِ انجام‌شده را باز نشان می‌دهد،
> یا لانچ را بی‌دلیل عقب می‌اندازد یا باعث دوباره‌کاری می‌شود.
>
> **وضعیتِ واقعی: از ۶ مورد، هر ۶ مورد بسته است** — با کد راستی‌آزمایی شد:
>
> | مورد | ادعای سرخطِ قبلی | واقعیت (تأییدشده) |
> |---|---|---|
> | ۳ Idempotency-Key در فرانت | باز | ✅ `apps/business/js/reservations.js:346,410` هدر را می‌فرستد |
> | ۵ اسکلت‌بودنِ push | باز | ✅ مهاجرتِ ۰۴۹ + ذخیره‌سازیِ واقعی. `ready` عمداً همیشه `false` است چون FCM/APNs ساخته نشده — یک محدودیتِ **مستند**، نه شکافِ باز |
> | ۷ نبودِ `error.tsx`/`not-found.tsx` | باز | ✅ هر دو در `api/src/app/` هستند |
>
> موردهای ۴، ۶، ۸ هم از قبل در PR #24 بسته شده بودند.
