...
| Row-Level Security on Postgres | n/a | n/a | n/a | FACT (live-tested 2026-08-12): only 15/49 tables get RLS from committed migrations; production DB reportedly had 35/35 as of 2026-07-19 via an undocumented manual step, **0 policies** either way | n/a | UNKNOWN | — | PARTIAL, two layers: (1) RLS is deny-by-default at the DB layer, but access control is actually enforced in application code (Prisma owner role bypasses RLS), so RLS never did active authorization work; (2) RLS coverage itself isn't reproducible from migrations for ~34 core tables — flagged as a P2/P3 finding for human decision, not fixed by an agent | `PROJECT-KNOWLEDGE.md` §2, `agency/knowledge/KNOWLEDGE_SYSTEM.md` §SECURITY_MEMORY |
