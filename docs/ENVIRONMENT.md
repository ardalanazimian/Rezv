...
| `SECRETS_KEYRING` | ✅ (prod) | — | api | AES-256-GCM keys for stored provider secrets (`platform_settings.zarinpal_merchant_id`, `webhooks.secret`; `api/src/lib/secret-box.ts`). Format `id:base64-32-bytes`, comma-separated for rotation. Unset or malformed in prod → the server refuses to start. Never stored in the DB. Losing it makes those stored values unreadable. |
| `SECRETS_ACTIVE_KEY_ID` | ✅ (prod) | — | api | Id in `SECRETS_KEYRING` that new values are sealed with. Rotation: add the new key, point this at it, restart, `POST /api/v1/maintenance/secrets-reseal`; remove the old key only after a run reports `rekeyed: 0` and `failed: []`. Immediately after first deploying migration 094, run it once with `?seal_plaintext=1`; until then payments and webhook deliveries fail closed by design, and gate A2 (`tools/gate-deploy.mjs`) requires the recorded response. |
