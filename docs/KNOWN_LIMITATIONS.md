...
### ۲u) فیچرِ بیعانه/پرداخت **کاملاً از جریانِ زنده قطع است** 🔴 (۲۰۲۶-۰۸-۲۲)

> **وضعیت: یافته‌ی باز.** بخشِ قابلِ‌رفعِ امنش رفع شد و کلِ ماژول تست‌دار شد،
> ولی خودِ اتصال عمداً انجام **نشده** — تصمیمِ محصولی/مالی است، نه فنی
> (پایین، «چرا رفع نشد»).

#### یافته

`lib/zarinpal.ts`، `POST /reservations/:code/pay` و `GET /payments/callback`
هر سه نوشته‌شده، دقیق و — همان‌طور که تست‌های جدید نشان می‌دهند — از نظرِ
منطقِ داخلی **درست** هستند. با این حال **هیچ مهمانی در تولید هرگز به آن‌ها
نمی‌رسد**، چون دو دروازه‌ی بالادستی هیچ‌وقت باز نمی‌شوند:

| دروازه | چه کسی باید آن را ست کند | چه کسی واقعاً ست می‌کند |
|---|---|---|
| `Reservation.depositRequested` / `depositAmountToman` | `resolvePolicy()` در `lib/cancellation-policy.ts` | **هیچ‌کس** — `resolvePolicy(` صفر فراخوان در کلِ `src/` دارد (فقط چند ذکرِ کامنتی) |
| `Restaurant.paymentEnabled` | یک روتِ تنظیماتِ رستوران یا ادمین | **هیچ‌کس** — نه روتی، نه seed، نه SQL؛ فقط `DEFAULT false` در مهاجرتِ ۰۲۰ |

نتیجه‌ی زنجیره‌ای، خط به خط در `pay/route.ts`:

```ts
if (!resv.restaurant.paymentEnabled) throw …            // همیشه false → همیشه throw
if (!resv.depositRequested || !resv.depositAmountToman) // همیشه false/null → همیشه throw
```

یعنی `POST /reservations/:code/pay` برایِ **هر رزروی که تا امروز ساخته شده**
۴۲۲ می‌دهد. این «باگِ منطقی» نیست — کد دقیقاً همان کاری را می‌کند که نوشته
شده؛ چیزی که وجود ندارد **بالادستِ** آن است.

#### چه چیزهایی به‌همین‌خاطر مرده‌اند

- **کلیدِ `deposit_required` در پنلِ بیزنس** (`apps/business/js/crm.js`) کار
  می‌کند، در جدولِ `CancellationPolicy` ذخیره می‌شود، و **هیچ اثری ندارد** —
  چون تنها خواننده‌اش `resolvePolicy()` است که صدا زده نمی‌شود. صاحبِ رستوران
  کلید را روشن می‌کند و باور می‌کند بیعانه فعال شده. این دقیقاً «دادهٔ جعلی
  که ادعای واقعی‌بودن دارد» است.
- **صفر UI در اپِ مشتری**: نه دکمه‌ی پرداخت، نه واژه‌ی «بیعانه»، و نه هیچ
  خواننده‌ای برایِ `?payment=paid|failed` که خودِ callback به آن ریدایرکت
  می‌کند (تأییدشده با grep روی کلِ `apps/customer/js/`).
- **`depositStatus` فقط نوشته می‌شود، هرگز خوانده نمی‌شود**: سه نقطه‌ی نوشتن،
  همه داخلِ همین دو روتِ پرداخت. هیچ منطقِ تأیید/لغوِ رزروی به آن نگاه نمی‌کند.
- **`DepositStatus.refunded` هیچ نویسنده‌ای ندارد** — یعنی مسیرِ بازگشتِ وجه
  اصلاً وجود ندارد.
- **هیچ jobِ انقضا/آزادسازیِ هولد** برایِ رزروی که بیعانه‌اش پرداخت نشده
  نوشته نشده (grep در `lib/` و `maintenance/`: چیزی نیست). اگر فردا این
  زنجیره وصل شود، رزروِ `deposit pending` تا ابد میز را نگه می‌دارد.

#### چرا این را به‌عنوانِ یافته ثبت می‌کنیم و خودسرانه وصل نمی‌کنیم

سه دلیل، به ترتیبِ اهمیت:

1. **تقابل با یک قانونِ صریحِ پروژه.** دستورِ ایستای «no payment processing»
   وجود دارد. دامنه‌ی دقیقش مبهم است (احتمالاً سفارشِ غذا/سبدِ خرید را هدف
   گرفته، نه بیعانه‌ی رزرو) — ولی وصل‌کردنِ چیزی که **پولِ واقعیِ مشتری** را
   می‌گیرد، جایی نیست که ابهام را خودم حل کنم.
2. **شکاف‌هایِ باز در خودِ جریان.** بدونِ مسیرِ بازگشتِ وجه، بدونِ انقضای
   هولد، و با شکافِ مستندشده‌ی «authorityِ یتیم» (رجوع به
   `docs/PROJECT-AUDIT-HANDOFF.md`)، وصل‌کردن یعنی پول‌گرفتن قبل از آماده‌بودنِ
   راه‌هایِ پس‌دادنش.
3. **`paymentEnabled` هم دروازه‌ی خودش را می‌خواهد** — حتی اگر
   `resolvePolicy()` وصل شود، بدونِ یک روتِ تنظیمات، هیچ رستورانی نمی‌تواند
   پرداخت را روشن کند مگر با SQLِ دستی.

#### چه چیزی در همین پاس **رفع** شد

- **باگِ واقعی و بی‌ابهام:** `payments/callback/route.ts` مقصدِ ریدایرکت را از
  `process.env.CUSTOMER_APP_URL` می‌خواند، در حالی که `.env.example` متغیرِ
  `NEXT_PUBLIC_CUSTOMER_APP_URL` را مستند می‌کرد. یعنی متغیرِ مستندشده هیچ‌وقت
  خوانده نمی‌شد و کد بی‌صدا رویِ دامنه‌ی هاردکدِ پیش‌فرض می‌افتاد — رویِ هر
  استقرارِ با دامنه‌ی متفاوت، ریدایرکتِ بعد از پرداخت به جایِ اشتباه می‌رفت.
  حالا از همان `appBase()` (`NEXT_PUBLIC_APP_URL`) می‌خواند که QRِ چک-اینِ
  میز هم از آن می‌خواند: یک متغیرِ کانونیک، نه سه نامِ موازی.
- **۳۱ تستِ جدید** (`tests/zarinpal.test.mts` ۱۳ تست با موکِ `global.fetch` —
  الگویی که در این ریپو وجود نداشت — و `tests/payments.integration.test.mts`
  ۱۸ تست رویِ Postgresِ زنده). پنج جهشِ عمدی در کدِ تولید امتحان شد و **هر پنج
  تا قرمز شدند**: حذفِ `currency:'IRT'`، حذفِ پذیرشِ `code===101`، حذفِ چکِ
  مالکیتِ رزرو، حذفِ تطبیقِ `authority`+`code`، و حذفِ خروجِ زودهنگامِ
  `already-success`.
- **قفلِ صریحِ وضعیتِ فعلی:** اولین تستِ فایلِ integration («رزروِ معمولی رد
  می‌شود») همین قطع‌بودن را ثبت می‌کند. اگر روزی کسی زنجیره را وصل کند، آن
  تست می‌شکند و مجبور می‌شود آگاهانه به‌روزش کند — نه اینکه بی‌صدا از کنارش رد
  شود.

#### پیش‌نیازهایِ وصل‌کردن (هر وقت تصمیم گرفته شد)

۱. یک روت برایِ `Restaurant.paymentEnabled` (پنلِ بیزنس یا ادمین).
۲. فراخوانیِ `resolvePolicy()` در مسیرِ ساختِ رزرو و نوشتنِ خروجی‌اش در
   `depositRequested`/`depositAmountToman`.
۳. UIِ پرداخت در اپِ مشتری + خواندنِ `?payment=paid|failed`.
۴. jobِ انقضایِ هولدِ رزروی که بیعانه‌اش pending مانده.
۵. مسیرِ بازگشتِ وجه (`DepositStatus.refunded` نویسنده ندارد).
۶. آشتیِ (reconciliation) authorityهایِ یتیم.

---

### ۲v) «اخراجِ کارمند» به‌عنوانِ کنترلِ امنیتی کار نمی‌کرد — **رفع شد** 🔴 (۲۰۲۶-۰۸-۲۲)

#### یافته

`withStaffAuth` در `lib/with-restaurant-auth.ts` **هیچ کوئریِ دیتابیسی نداشت** —
فقط امضایِ JWT را بررسی می‌کرد. کامنتِ خودش این را «نسخه‌ی سبک‌تر» می‌نامید و
دلیلِ منطقی هم داشت (نباید `resolveStaffRestaurant` را صدا بزند، چون آن تابع
برایِ تنانتِ بدونِ رستوران ۴۰۴ می‌دهد) — ولی «سبک» به «بدونِ هیچ اعتبارسنجیِ
وضعیت» ترجمه شده بود.

تنها مصرف‌کننده‌اش `restaurant/staff/route.ts` است: **همان endpointی که کارکنان
را اضافه و غیرفعال می‌کند.** `assertManagerOrOwner` هم فقط `auth.role` را از
**توکن** می‌خواند.

نتیجه: مدیری که همین حالا اخراج شده بود (`is_active=false`)، تا ۱۵ دقیقه — عمرِ
accessِ بدونِ هیچ لیستِ ابطالی — هنوز کاملاً کار می‌کرد:

| کار | اثر |
|---|---|
| `PATCH` با `staff_id` خودش و `is_active: true` | 🔴 **خودش را دوباره فعال می‌کرد** — پنجره‌ی ۱۵ دقیقه‌ای به دسترسیِ **دائمی** تبدیل می‌شد |
| `POST` کارمندِ جدید با شماره‌ی خودش | درِ پشتیِ ماندگار، مستقل از توکنِ فعلی |
| `GET` فهرستِ کارکنان | نشتِ نام و شماره‌ی موبایلِ همه‌ی کارکنانِ تنانت |
| `PATCH` رویِ دیگران | غیرفعال‌کردنِ همکاران، تغییرِ دسترسی‌ها، جابه‌جاییِ شعبه |

سطرِ اول از همه بدتر است: گاردهایِ خودِ `PATCH` فقط جهتِ `is_active === false`
را می‌گرفتند («نمی‌توانید خودتان را غیرفعال کنید»)، و برایِ جهتِ `true` هیچ
شرطی نبود.

**این حدس نیست:** تستِ `🔴 مدیرِ اخراج‌شده نمی‌تواند خودش را دوباره فعال کند`
رویِ کدِ قبل از رفع اجرا شد و **قرمز شد** (۷ از ۱۵ تستِ فایل رویِ کدِ قدیمی
می‌افتند)، سپس رویِ کدِ رفع‌شده سبز.

#### چرا خواهرش سالم بود

`withRestaurantAuth` این شکاف را نداشت، چون `resolveStaffRestaurant` را صدا
می‌زند که از ۲۰۲۶-۰۸-۲۰ عضویتِ تنانت و `isActive` را با DB تطبیق می‌دهد.
**دو درِ یک اتاق، یکی سفت و دیگری نه** — دقیقاً همان الگویِ باگِ گاردِ ادمین
(`admin-auth.ts`) و مقایسه‌ی غیرِثابت‌زمانِ `maintenance-auth.ts` در همین جلسه.
سه بار تکرارِ یک الگو: **وقتی یک چکِ امنیتی اضافه می‌شود، باید همه‌ی مسیرهایِ
هم‌ارز پیدا و هم‌زمان بسته شوند، نه فقط مسیری که در دست است.**

#### رفع (دو لایه)

۱. `withStaffAuth` حالا `verifiedStaffAuth()` را صدا می‌زند: یک کوئریِ سبکِ
   `db.staff.findUnique` که وجودِ حساب، `isActive` و تطبیقِ `tenantId` را چک
   می‌کند — بدونِ اینکه وجودِ رستوران را لازم کند (نیتِ اصلیِ طراحی حفظ شد).
   `role` هم از **DB** برمی‌گردد و همان به handler داده می‌شود، نه ادعای توکن.
   (امروز مسیرِ APIی برایِ تغییرِ نقش وجود ندارد پس این بخش قابلِ سوءاستفاده
   نبود؛ ولی گارد باید قبل از اضافه‌شدنِ چنین مسیری درست باشد.)
۲. گاردِ `PATCH` حالا **هر** تغییرِ وضعیتِ فعال‌بودنِ خودِ کاربر را رد می‌کند،
   در هر جهتی — نه فقط `false`. دفاعِ عمقی؛ لایه‌ی اصلی همان بندِ ۱ است.

#### پوششِ تست

`tests/staff-auth-guard.integration.test.mts` — ۱۵ تست رویِ Postgresِ زنده با
فراخوانیِ روت‌هایِ واقعی: هر چهار سطرِ جدولِ بالا، به‌علاوه‌ی حسابِ حذف‌شده،
جعلِ عضویتِ تنانت، ادعایِ نقشِ کاذب در توکن، کنترلِ مثبت (مدیرِ سالم باید کار
کند)، و اینکه قابلیت‌هایِ درست نشکسته باشند (مالک هنوز می‌تواند مدیر را
غیرفعال کند). یک تست هم خودِ سورس را قفل می‌کند تا اگر کسی کوئریِ DB را
بردارد همان‌جا بشکند — چون ریشه‌ی باگ «نبودِ کوئری» بود، نه یک شرطِ خاص.

---

### ۲w) اشتراکِ منقضی تا ۲۴ ساعت «رو به اتمام» گزارش می‌شد — **رفع شد** 🟡 (۲۰۲۶-۰۸-۲۲)

#### یافته

`computeSubscriptionStatus` تصمیمِ «منقضی شده یا نه» را رویِ عددِ **گِردشده**
می‌گرفت:
...
`Math.ceil(-0.5)` برابرِ `-0` است، و در جاوااسکریپت **`-0 < 0` نادرست است**.
پس اشتراکی که هر لحظه‌ای در ۲۴ ساعتِ گذشته منقضی شده بود از این شرط رد می‌شد و
یک خط پایین‌تر در دامِ `diffDays <= 14` می‌افتاد → **«رو به اتمام» با «۰ روز
باقی‌مانده»**. `-0` در JSON هم `0` سریالایز می‌شود، پس در پاسخِ API حتی
نشانه‌ای هم باقی نمی‌ماند.

#### اثر

گزارشی، نه دسترسی — هیچ گاردی به این وضعیت تکیه نمی‌کند (فقط دو نمایِ
خواندنیِ پنلِ شرکت: `admin/overview` و `admin/restaurants`). ولی دقیقاً از جنسِ
«گزارشِ وضعیتی که واقعی نیست»: مدیرِ پلتفرم که فهرستِ تمدید را از همین صفحه
برمی‌دارد، **هر تنانتی را که در روزِ اولِ انقضا بود از قلم می‌انداخت** — و
شمارنده‌ی `expired` در داشبورد هم به همان اندازه کم‌شمار بود.

#### رفع

تصمیم رویِ **اختلافِ خامِ میلی‌ثانیه** گرفته می‌شود (بدونِ گِردکردن)؛ گِردکردن
فقط برای *نمایش* می‌ماند و `-0` به `0` نرمال می‌شود تا ناسازگاریِ `Object.is`
به بیرون درز نکند.

همراهش یک **پارامترِ مرده** حذف شد: `plan` در امضای تابع بود ولی در بدنه هرگز
خوانده نمی‌شد — پارامترِ مرده‌ی بامعنا-به‌نظر گمراه‌کننده است (خواننده فرض
می‌کند نوعِ پلن روی وضعیت اثر دارد). هر دو فراخوان‌کننده به‌روز شدند و
کامپایلر تأیید کرد که جایی جا نیفتاده.
...
## ~~فیدِ «محبوب امشب» به ترتیبِ UUID مرتب می‌شود~~ — ✅ رفع شد

**تاریخ:** ۲۰۲۶-۰۸-۲۵ · **وضعیت:** **بسته** (مهاجرتِ ۰۷۳، همان روز)

### شرح

عنوانِ پیش‌فرضِ فیدِ اپِ مشتری «🔥 محبوب امشب» است
(`apps/customer/js/data/discover.js` — تابعِ `filterVibe`، حالتِ `v==='all'`،
و همان عنوان در بازگشت از فیلترِ موقعیت). ولی ترتیبِ واقعیِ لیست از
`GET /api/v1/restaurants` می‌آید و آن‌جا:

```ts
orderBy: { id: 'desc' },           // ترتیب پایدار برای cursor
```

`id` یک UUID است. یعنی ترتیب **پایدار** هست (که برایِ cursor لازم است و همان
هدفِ اصلیِ این خط بود) ولی هیچ ربطی به محبوبیت ندارد — عملاً تصادفی است.

### چرا این یک یافته است، نه سلیقه

سیگنالِ واقعیِ محبوبیت **از قبل در همین route حساب می‌شود**: `visitRows` تعدادِ
رزروهایی است که در ۷ روزِ گذشته به حضور رسیده‌اند
(`db.reservation.groupBy` با `visitedStatusList()`)، و همان عدد به‌صورتِ
`visits7d` روی هر کارت هم نمایش داده می‌شود. مشکل ترتیبِ عملیات است: این
`groupBy` **بعد از** صفحه‌بندی و فقط روی idهای همان صفحه اجرا می‌شود، پس
نمی‌تواند مبنایِ مرتب‌سازی باشد.

یعنی محصول عددِ درست را دارد، نشانش هم می‌دهد، ولی بر اساسش مرتب نمی‌کند —
و همزمان به کاربر می‌گوید این فهرست «محبوب امشب» است. این دقیقاً همان کلاسِ
عدمِ‌صداقتی است که این مخزن قبلاً دو بار بسته: امتیازِ ۴٫۸ برایِ رستورانِ
بدونِ نظر، و فاصله‌ی ثابتِ «۰٫۷ کیلومتر».

### چرا الان رفع نشد

رفعِ درست یک ستونِ denormalizedِ `visits_7d` روی `restaurants` می‌خواهد
(به‌همراه ایندکسِ `(is_open, visits_7d DESC, id DESC)`، یک مهاجرتِ SQL،
همگام‌سازیِ `schema.prisma`، و یک جابِ cron برایِ بازمحاسبه) — چون
`orderBy` روی نتیجه‌ی `groupBy` یک جدولِ دیگر در Prisma ممکن نیست و
صفحه‌بندیِ cursor باید روی همان ترتیبِ تازه بازطراحی شود.

این تغییرِ اسکیما در لحظه‌ای پیشنهاد شد که سه ایجنت هم‌زمان روی `api/`
می‌نوشتند؛ شروعِ یک مهاجرت در آن وضعیت ریسکِ بی‌موردی به کلِ برنچ اضافه
می‌کرد. عمداً به تعویق افتاد، نه اینکه ناشناخته بماند.

### دو مسیرِ ممکن برایِ تصمیم‌گیرنده

۱. **ساختنِ وعده (ترجیح):** ستون + ایندکس + cron، بعد
   `orderBy: [{ visits7d: 'desc' }, { id: 'desc' }]`. عنوان درست می‌ماند.
۲. **گفتنِ حقیقت (موقت):** عنوانِ پیش‌فرض به چیزی صادقانه مثل «رستوران‌ها»
   تغییر کند تا وعده‌ای داده نشود که پشتش نیست.

### آنچه در نهایت انجام شد (مسیرِ ۱ — ساختنِ وعده)

مهاجرتِ ۰۷۳ ستونِ `visits_7d` را به `restaurants` اضافه کرد، به‌همراهِ ایندکسِ
`(is_open, visits_7d DESC, id DESC)` که دقیقاً ترتیبِ همان کوئری است.
`lib/restaurant-popularity.ts` آن را از رویِ رزروهایی که واقعاً به حضور
رسیده‌اند بازمحاسبه می‌کند (همان `VISITED_RESERVATION_STATUSES` که عددِ روی
کارت را می‌سازد — تا دو عدد واگرا نشوند)، و جابِ شبانه‌ی `customer-insights`
صدایش می‌زند. ترتیبِ route به
`[{ visits7d: 'desc' }, { id: 'desc' }]` تغییر کرد؛ `id` شکنندهٔ تساوی ماند
تا صفحه‌بندیِ cursor سالم بماند.

**اثبات:** ۴/۴ سبز، شاملِ کنترلِ منفی (با عوض‌شدنِ داده ترتیب هم عوض می‌شود).
جهش‌آزمایی: بازگشت به `id DESC` ← ۲ قرمز؛ بازکردنِ پنجره‌ی ۷ روزه به ۹۰ روز
← ۲ قرمز.

⚠️ **محدودیتِ باقی‌مانده، صادقانه:** این ستون در جابِ شبانه (۰۳:۰۰)
بازمحاسبه می‌شود، پس تا ۲۴ ساعت کهنه است. برای یک شمارشِ **۷ روزه‌ی غلتان**
بی‌اهمیت است (عدد در طولِ یک روز جهش نمی‌کند)، ولی ادعای «همین لحظه» نیست و
عنوانِ «امشب» دقیقاً یک روزه هم نیست. اگر روزی تازگیِ ساعتی لازم شد، همان
تابع را می‌شود در یک cronِ کوتاه‌تر هم صدا زد — هزینه‌اش یک `UPDATE` روی کلِ
جدول است.

---

## ۳۱ فایلِ تست یک سطلِ ریت‌لیمیتِ مشترک دارند

**تاریخ:** ۲۰۲۶-۰۸-۲۵ · **وضعیت:** باز — یکی رفع شد، ریشه‌ی سیستمی باقی است

### شرح

`clientIp()` وقتی هیچ هدرِ IPی نباشد رشته‌ی **ثابتِ** `'unknown'` برمی‌گرداند
(`api/src/lib/ratelimit.ts`). هر فایلِ تستی که `new Request()` بدونِ هدر
بسازد، در همان یک کلیدِ Redis جمع می‌شود: `rl:auth:unknown`. سقفِ سطحِ
`auth` هم `max: 20, windowMs: 60_000` است.

امروز **۳۱ فایل** در `api/tests/` این وضع را دارند (چکِ ساده: `new Request(`
دارند ولی هیچ‌جا `x-real-ip`/`x-forwarded-for`/`cf-connecting-ip` ندارند).

### چرا این یک بمبِ ساعتی است، نه صرفاً بی‌نظمی

سطل مشترک است، پس **هر** تغییری که ترافیکِ سطحِ `auth` را زیاد کند، باعث
می‌شود یک تستِ **کاملاً بی‌ربط** قرمز شود. دقیقاً همین رخ داد: بعد از
انتقالِ دو روتِ دستیار از `search` به `auth`، تستِ «تلفنِ نامعتبر → ۴۲۲»
در `member-create` با **۴۲۹** قرمز شد و کلِ جابِ `test` در CI افتاد —
در حالی که آن فایل اصلاً دست نخورده بود.

خطرناک‌تر: چون به زمان‌بندی و ترتیب وابسته است، محلی سبز می‌شود و در CI
قرمز. (همین‌طور هم شد: اجرای محلیِ من ۱۳۴۴/۱۳۴۴ بود.)

### بازتولیدِ کنترل‌شده (اجرا شد، حدس نیست)

سطلِ `rl:auth:unknown` را با ۲۵ عضو پر کن، سپس همان فایل را اجرا کن:

* بدونِ `x-real-ip` ← تست‌ها `429` می‌گیرند (همان علامتِ CI)
* با `x-real-ip`ِ یکتا ← ۵/۵ سبز

### آنچه انجام شد

`member-create.integration.test.mts` (فایلی که واقعاً شکست) به IPِ یکتا
منتقل شد، و کمکیِ مشترکِ `api/tests/helpers/test-ip.mts` ساخته شد تا الگو
یک‌جا مستند و در دسترس باشد.

### آنچه انجام **نشد** و چرا

۳۰ فایلِ دیگر منتقل نشدند. این کار مکانیکی ولی پرحجم است و در لحظه‌ای که
سوئیت تازه سبز شده بود، ریسکِ بی‌موردی به شاخه اضافه می‌کرد. هرکدام که
بعداً قرمز شود، رفعش یک خط است: `'x-real-ip': testIp()`.

⚠️ **راهِ حلی که عمداً انتخاب نشد:** «پاک‌کردنِ کلیدهای `*auth*` در
`before`». چند فایل همین کار را می‌کنند و **مضر** است: سطلِ فایل‌های دیگرِ
همین رانر را هم خالی می‌کند و ریت‌لیمیت را از تستِ آن‌ها پنهان می‌کند —
یعنی یک گاردِ امنیتیِ واقعی را از سنجش خارج می‌کند. IPِ یکتا سطل را
**جدا** می‌کند نه خالی، پس سقفِ واقعیِ روت همچنان سنجیده می‌شود.

---

## رویدادهای سروریِ `site.*` سطحِ اعتمادِ کلاینت می‌گیرند

**تاریخ:** ۲۰۲۶-۰۸-۲۶ · **وضعیت:** باز — نیازِ تصمیمِ معماری

پنج رویدادِ `site.*` در `api/src/lib/site-orders.ts` (سفارشِ فروش،
فعال‌سازیِ اشتراک، …) `recordEvent` را **بدونِ `trustLevel`** صدا می‌زنند،
پس پیش‌فرضِ `ANONYMOUS_CLIENT` می‌گیرند.

اینها رویدادهایی هستند که **خودِ بک‌اند** تولید می‌کند — نه ورودیِ
غیرقابلِ‌اعتمادِ مرورگر. دو پیامد:

۱. در سطلِ هرسِ ۹۰ روزه می‌افتند به‌جای ۴۰۰ روزه (`retentionDays()`)، پس
   دادهٔ فروش زودتر از آنچه باید پاک می‌شود.
۲. هرگز واجدِ شرایطِ آموزش شمرده نمی‌شوند.

**چرا رفع نشد:** رفعش قاعده‌ی ارتقای `trust_level` را جابه‌جا می‌کند —
یعنی معنیِ یک ستونِ موجود را برای ردیف‌های گذشته و آینده عوض می‌کند.
تصمیمِ معماری است، نه باگ‌فیکسِ محلی.

**کمترین رفع:** در `site-orders.ts` هنگامِ `recordEvent` صریحاً سطحِ
سرور-تأییدشده پاس داده شود. پیش از آن باید مشخص شود ردیف‌های موجود
migrate می‌شوند یا نه.

---

## دو فایلِ تست روی `jobs` مسابقه می‌دهند

**تاریخ:** ۲۰۲۶-۰۸-۲۶ · **وضعیت:** باز — فقط در اجرای موازیِ چندفایلی

`notification-consent-enforcement.test.mts` **کلِ** ردیف‌های `jobs` با
`kind='sms'` را سراسری `deleteMany` می‌کند، و
`sms-queue-fallback-balance.test.mts` هم `count({kind:'sms'})`ِ سراسری
می‌شمارد. در اجرای موازیِ چندفایلیِ `tsx --test` با هم مسابقه می‌دهند.

⚠️ زیرِ `_all.runner.mts` (تک‌پروسه و ترتیبی — همان چیزی که `npm test` و CI
اجرا می‌کنند) رخ **نمی‌دهد**، پس یک flakeِ زنده نیست. ولی همان خانواده‌ی
«وضعیتِ سراسریِ مشترک» است که امروز سه بار هزینه داد، و اگر روزی رانر
موازی شود فوراً فعال می‌شود.

**کمترین رفع:** هر دو فایل دامنه‌ی کوئریشان را به رستوران/تنانتِ خودشان
محدود کنند، نه `kind` سراسری.

---

## `npm run test:one` می‌تواند «سبزِ ناقص» گزارش کند

**تاریخ:** ۲۰۲۶-۰۸-۲۶ · **وضعیت:** باز — روی CI اثر ندارد، روی راستی‌آزماییِ دستی چرا

`npm run test:one` یعنی `tsx --test --test-force-exit`. آن پرچم پروسه را
پیش از پایانِ سوئیت‌های آخر می‌کُشد و شمارنده‌ها **بی‌صدا کم می‌شوند** —
همیشه با `# fail 0` و `# cancelled 0`، پس هیچ نشانه‌ای از ناقص‌بودن نیست.

**بازتولیدِ اجراشده** روی `tests/validate.test.mts` (۳۹ تست):

```
با --test-force-exit :  39  35  39  39  39
بدونِ آن پرچم        :  39  39  39
```

یعنی یک اجرا **۳۵** گزارش کرد و سبز به‌نظر رسید.

⚠️ **چرا مهم است:** این دقیقاً هم‌خانواده‌ی حادثه‌ی «۳۷۵ گزارش شد، واقعیت
۳۵۲ بود» در بخشِ ۷ CLAUDE.md است. `npm test` این پرچم را **ندارد**، پس CI
درگیر نیست؛ ولی `test:one` همان دستوری است که CLAUDE.md برای «تأییدِ یک
فایل» توصیه می‌کند و هر آدم/ایجنتی برایش استفاده می‌کند.

**چرا پرچم اصلاً هست:** هندلِ بازِ Redis پروسه را زنده نگه می‌دارد و بدونِ
آن `tsx --test` هرگز exit نمی‌کند (همان چیزی که CLAUDE.md بخشِ ۴ هشدار
می‌دهد). پس نمی‌شود صرفاً برش داشت.

**کمترین رفع:** بستنِ صریحِ اتصالِ Redis در یک teardown سراسری، بعد حذفِ
پرچم. تا آن‌وقت: برای هر ادعای مهم، عددِ تست را با اجرای بدونِ پرچم (و
timeout) تأیید کن، نه فقط `fail 0`.

---

## هیچ گیتِ ایستایی روی `api/tests/*.mts` وجود ندارد

**تاریخ:** ۲۰۲۶-۰۸-۲۶ · **وضعیت:** باز

* `eslint.config.mjs` صراحتاً `tests/**` را ignore می‌کند.
* `tsconfig.json` فقط `**/*.ts` و `**/*.tsx` را include می‌کند — پسوندِ
  `.mts` **اصلاً** تایپ‌چک نمی‌شود.

**کنترلِ مثبتِ اجراشده:** خطِ
`const __probe: number = "definitely-not-a-number";` به
`tests/jwt.test.mts` اضافه شد ⇒ `npx tsc --noEmit` **صفر خطا** داد.

یعنی تنها گیتِ واقعیِ فایل‌های تست، **اجرا شدنشان** است. هر خطای نوعی که
مسیرِ اجرا به آن نرسد، تا ابد نامرئی می‌ماند.

---

## دو فایلِ تست، ریت‌لیمیت را برای کلِ سوئیت خاموش می‌کنند

**تاریخ:** ۲۰۲۶-۰۸-۲۶ · **وضعیت:** باز — نیازِ رفع

`sms-transport-failclosed.integration.test.mts:43-49` و
`otp-ratelimit-and-deadlock.integration.test.mts:54-62` هرکدام یک
`beforeEach` در **سطحِ فایل** دارند که `redis.keys('*rl:*')` را پاک می‌کند.

در `node:test` هوکِ سطحِ فایل به سوئیتِ **ROOT** می‌چسبد، یعنی پیش از
**هر تستِ کلِ رانر** اجرا می‌شود و همه‌ی سطل‌های ریت‌لیمیت را می‌شوید.

پیامد دولایه است: هم باگِ سطلِ مشترک را سال‌ها پنهان می‌کرد، و هم **هر
ادعای ریت‌لیمیت در کلِ سوئیت را بی‌اثر می‌کند** — تستی که می‌گوید «سقف
اعمال می‌شود» ممکن است فقط به این دلیل سبز باشد که سطل هر بار خالی شده.

**کمترین رفع:** انتقالِ هوک به داخلِ `describe` + محدودکردنِ الگو به
کلیدهای همان فایل. (همان الگویی که در ۳۱ فایلِ دیگر امروز اعمال شد.)

⚠️ این سومین مصداقِ «هوکِ سطحِ فایل به ROOT می‌چسبد» در همین یک نشست است
(`email-transport-honesty` با `globalThis.fetch`، و
`password-login` با `PLATFORM_ADMIN_TENANT_ID`).

---

## pre-order فقط qty=1 می‌فرستد — مشتری نمی‌تواند دو پرس از یک آیتم سفارش دهد

**تاریخ:** ۲۰۲۶-۰۸-۲۷ · **وضعیت:** محدودیتِ آگاهانه‌ی scope (نه باگ) — ثبت به دستورِ مالک

* **سرور آماده است:** قراردادِ `POST /reservations` برای هر ردیفِ preorder
  «qty» را `int().min(1).max(50)` می‌پذیرد (`reservations/route.ts:31`) و
  `checkout.subtotal` همان qty را ضرب می‌کند (`lib/reservations.ts:583`).
* **UI نمی‌فرستد:** چیپ‌های گامِ ۲ فقط toggleِ انتخاب‌اند و payload همیشه
  `{menu_item_id, qty: 1}` است (`apps/customer/js/data/booking.js:212`) —
  شمارنده/stepperی وجود ندارد.
* **چرا این‌طور شد:** SPEC-A (سندِ مصوبِ `docs/specs/SPEC-A-menu.md`) هیچ‌جا
  UI کمیت را تعریف نکرده — کلمه‌ی qty حتی یک بار در spec نیامده. فازِ ۲ دقیقاً
  scope مصوب را ساخت: «سیم‌کشیِ واقعیِ چیپ‌های موجود»، نه طراحیِ ورودیِ تازه.
  پس «تصمیمِ صریحِ محصولی برای منعِ دو پرس» نبود؛ مرزِ scope بود که اثرِ
  جانبی‌اش این محدودیت است.
* **اثرِ کاربری:** دو پرسِ یک آیتم = دو بار همان آیتم قابلِ انتخاب نیست؛
  کاربر فقط می‌تواند آیتم‌های متفاوت برگزیند. مبلغِ پیش‌سفارش و «+۲۰ امتیاز»
  درست می‌مانند، فقط سبد از هر آیتم حداکثر یکی می‌گیرد.

**مسیرِ باز شدن (وقتی محصول خواست):** فقط UI — stepper روی چیپِ انتخاب‌شده و
ارسالِ همان عدد؛ هیچ تغییرِ سرور/DB لازم نیست (سقف ۵۰ همین حالا اعمال می‌شود).
