...
> ⚠️ **مهم‌ترین نکته‌ی این سند:** رزرونو **یک** دیزاین‌سیستم ندارد، **سه** سطحِ
> CSSِ موازی دارد که هیچ‌کدام به‌هم sync نمی‌شوند. پیش از هر کاری بخشِ ۰ را بخوان.

> **اصلاح‌شده (۲۰۲۶-۰۸-۱۲):** این سند تا آن روز همه‌جا «دنیایِ B» را
> `apps/seo` می‌نامید — از قبل از جدا شدنِ وب‌سایتِ عمومی به `apps/landing`ِ
> مستقل (ADR 0002، بازنگری‌شده). جزئیاتِ فنیِ سند (تعدادِ آیکن ۴۴، لیستِ
> کامپوننت‌ها، نسخه‌های Next/React) با `apps/landing` تأیید و مسیرها اصلاح
> شدند — خودِ محتوا درست بود، فقط اسمِ اپ اشتباه.

> ⚠️ **اصلاحِ دوم (۲۰۲۶-۰۸-۲۰):** اصلاحِ بالا با تأکید اضافه می‌کرد «چک‌شده:
> `apps/seo` امروز اصلاً فایلِ CSS یا `Icon.tsx` ندارد». آن جمله **دیگر درست
> نیست**: `apps/seo/app/globals.css` در PR #32 (کامیتِ `99655c1`) اضافه شد و در
> `apps/seo/app/layout.tsx` خطِ ۵ ایمپورت می‌شود. پس دنیاها **سه** تا شدند، نه دو.
> `apps/seo` هنوز `Icon.tsx` ندارد — فقط ادعای «هیچ CSSی ندارد» باطل شده.
>
> درسِ تکرارشونده: یک ادعای «چک‌شده» فقط تا تاریخِ همان چک معتبر است. قبل از
> تکیه به هر ادعای این سند، خودت با `ls`/`grep` تأییدش کن.

---

## ۰) اول این را بدان: سه دنیای جدا

| | دنیای A — پنل‌ها | دنیای B — وب‌سایت | دنیای C — صفحاتِ SEO |
|---|---|---|---|
| اپ‌ها | `apps/customer` · `apps/business` · `apps/company` | `apps/landing` | `apps/seo` |
| فناوری | HTML + CSS + JS خام | Next.js 16 + React 18 + TypeScript | Next.js + React |
| مرحله‌ی build | **ندارد** — فایل مستقیم سرو می‌شود | دارد (Turbopack) | دارد |
| منبعِ توکن | `shared/css/tokens.css` | `apps/landing/app/globals.css` | `apps/seo/app/globals.css` |
| توزیعِ توکن | `tools/sync-design-system.sh` کپی می‌کند | کپی نمی‌شود — مستقل است | کپی نمی‌شود — مستقل است |
| آیکن | `shared/js/icons.js` (رشته‌ی SVG) | `components/site/Icon.tsx` (React) | **ندارد** |
| ADR | — | 0002 | 0001 |

**توکن‌های هم‌نام** (`--sp-4`, `--radius-md`, `--brand-500`) در فایل‌های جدا تعریف
شده‌اند. تغییرِ یکی هیچ اثری روی بقیه ندارد.

پیش از تولیدِ کد از Figma، اول تعیین کن مقصد کدام دنیاست:
- طرحِ صفحه‌ی بازاریابی/محتوایی → **دنیای B**
- طرحِ پنلِ عملیاتی → **دنیای A**
- طرحِ صفحه‌ی عمومیِ یک رستوران (`/r/[slug]`) → **دنیای C**

⚠️ بقیه‌ی این سند عمداً فقط دنیای A و B را با جزئیات پوشش می‌دهد — دنیای C
کوچک است و دیزاین‌سیستمِ بالغی ندارد. اگر مقصد C است، اول
`apps/seo/app/globals.css` را بخوان؛ فرض نکن توکن‌هایش با A/B یکی است.
