...
⚠️ **یک تضعیفِ آگاهانه:** نسخه‌ی api می‌گفت آخرین تیغه **دقیقاً** در ۱×`--intro-t` تمام می‌شود (`|end−1|<1e-9`)؛ نسخه‌ی باقی‌مانده می‌گوید **نه دیرتر** (`≤ 1`). نقص «پرده وسطِ کار قطع می‌شود» فقط با دیرتر رخ می‌دهد؛ زودتر تمام‌شدن نقص نیست. اگر بازبین «دقیقاً» را طراحی می‌داند نه سلیقه، یک خط است.
