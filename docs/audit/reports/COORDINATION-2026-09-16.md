...
| Sync `rezv-7a` (SYNC + MERGE-BACKLOG) | 13 | 10 | **5 wrong → all corrected by the author**; **a 6th, found later by `rezv-1b` (`cddf476`):** the "`shared/js` 4 of 5 diverged, api-core +7/-5" in SYNC §4F is the sync tool's export-strip transform, not divergence (the tool owns all 3 copies; `--check` exit 0; `cmp` after sync shows only the edited lines). Refined by `rezv-75` (`e14a23d`): after normalizing the export-strip, customer is byte-identical and business/company differ by exactly 2 lines, a `window.*` globals shim for their classic-script load. So there is no logic divergence, and the transform is export-strip plus the shim; (C-8, C-9; 23/106/23 conflict counts miscounted, the root cause was its own `tail -n +2` taking message lines; the 048 "collision" retracted) | C-8, C-9; its re-run is accepted |
| Red Team `rezv-31` | 8 (4/4 blockers) | 8 | 0 | caught my own citation error |
| Implementation `rezv-85` | 12 | 11 | **1 wrong → self-reported and corrected** (the b496991 "18 guards exit 0" was a false green: `$?` read after a printf substitution, 2 guards actually red; fixed at `4c72f2a`, re-measured exit-code-first with a positive control) | the 8cdc6e9 and 553ecf4 claims hold on re-measure; my own c234479 run was not affected |
...
| D-31 | **Per-restaurant deep link is launch-path, plus slug diacritics fixed.** `rezv-75` gets a fix order (its charter allows fixes on CEO order; it doesn't certify; Red Team attacks, including XSS/open-redirect on slug and params). `slugSeed` strips U+064B..U+0652 and U+0670 while keeping Persian letters. One canonical shareable restaurant URL that preserves date and party, reused by SEO «رزرو» and table QR, with honest not-found and not-bookable states | `rezv-75` `1f5a128`: an un-typeable slug, journey 4 incomplete; the diners playbook routes launch diners through restaurant-owned links | yes |
| D-30 | **Private repository `ardalanazimian/Rezv-security` (owner ruling 09-17).** It holds the red-team material from main, the 5 `redteam/*` branches and Red Team's unpushed commits, plus the investment docs (`INVESTORS.md` in both versions, `THE-ASK.md`). Import `591947c` with `MANIFEST.tsv`. Public `main` `592bfca` replaces 11 files with "moved" tombstones, keeping paths so references still resolve. The 5 public `redteam/*` branches were deleted and pinned locally under `refs/snap/rezv87-0917-deleted/`. The Red Team charter now points output to the private repo. **Residual:** everything already pushed remains in public git history | fresh clone: 47/47 blobs equal the manifest; unauthenticated API → 404; 20 guards 0 on the tombstone tree; the deleted branches' non-redteam diffs were only stale ROUTING rows | the deletion is not reversible from GitHub, but it is from the local pins |
| D-29 | **P1-4 Option A re-rulings** (`rezv-85` design `82d8b3c`). **R-1 amends D-23, and the error was mine:** a recorded OTP login is NOT acceptance evidence, because a hijack writes exactly that record. Backfill accepts only owner rows with their own ACCEPTED StaffInvite. R-2: password login never accepts (a password proves a username, not a phone). R-3: 1 accepted + pending gets the chooser, with the accepted row pre-selected. R-4: acceptance at login only; JWT_REFRESH_SECRET rotation at deploy goes into the A2 checklist. R-5: 079 applies to accepted owners only. R-6: `invite.html` copy is inside the change. R-7: a resubmit returns no order fields; landing `PurchaseDialog`/`TrialForm` edits are authorized inside P1-4 (no Designer alive) | `P1-4A-DESIGN.md`; `provisioning.ts:265-283`; `login/route.ts:40` | yes |
| D-28 | **M-21 subscription design approved.** Q1: complimentary accounts carry an expiry and admin create requires one. Q2: walk-ins and manual reservations locked when expired. Q3: tables and club member mutations locked; review replies open. Q4: no waitlist offers. **Q5: expired tenants HIDDEN from discovery** (overrides D-27's wording: showing them would be the deferred free listing); a direct link renders honestly. The self-serve trial goes to the owner, with a recommendation to remove it | premises checked on main: `subscription.ts:43,69-70`, `site-orders.ts:36,301-306,573-580`, `reservations/route.ts:64-69`, `notifications/route.ts:12` | yes |
| D-27 | **Subscription lifecycle defaults** (owner can override): active or complimentary (plan plus expiry set by an admin) gets full service. ≤15 days left: a persistent banner with the exact days plus a one-time panel notification. Expired: profile, menu and campaign edits locked; NEW online bookings closed (no dead button for diners); existing reservations stay manageable by staff; data is never deleted | owner decisions 09-17 (paid launch, team-created accounts, 15-day notice) | yes |
| D-26 | **F001 catch-up tick** (cron down ≥ grace, so `running_late` is set after `guestDeadline`): no `booking_late` SMS, no auto no-show, a "catch-up without warning" metric increments, and staff decide by hand under D-20(c), unchanged. Options (a) accept and (c) move the token and staff gate were rejected | `rezv-1b` R1 | yes |
| D-24 | **S-05 scope and treatment:** one workstream, two treatments by class. Readable secrets (webhook HMAC secret, merchant id) are encrypted with AES-256-GCM plus an env keyring and key id, fail-closed. Compared-only bearer tokens (invite token) are hashed one way. An idempotent in-place migration covers existing rows; pre-migration invite links still claim. Rotation is proven by rotating twice | `rezv-85` inventory, verified: `schema.prisma:1265-1266,1365`, `sms.ts:279-280,317` | yes |
| D-23 | **P1-4 (staff/owner phone hijack): Option A, consent at login.** SPEC-B C10 ("acceptance is a side effect of first login") is a spec clause, not an ADR, so changing it is inside CEO authority; recorded as a deliberate contract change. Migration 093; the backfill counts as accepted only with evidence; 079 uniqueness on accepted owners only; single-membership response shape preserved. B rejected (unproven SMS plus phishing surface), C rejected (not a fix). I-1 approved now. Queue re-ordered with blockers first: I-1 → S-05 → P1-4 A → P1-5 → m-14 | `rezv-85` package `feea34b` (repro 5 tests, 4 fail); premises checked: `site-orders.ts:288`, `provisioning.ts:121`, `site/trial/route.ts:40` | yes |
| D-22 | **F002 re-verification criterion rescoped** (`rezv-75`'s objection, accepted): certify the card per state, the SMS tokens, and no contradiction with recorded facts (strike, cashback reversal, actor). **Card wording versus SMS wording is UNKNOWN,** because the template bodies live in the Melipayamak panel (`sms.ts:55-72` sends token values only). The partial anchor is the label token at `lifecycle.ts:69` | `sms.ts:55-72`; `lifecycle.ts:69,294-296` | yes |
| D-21 | **D-19's scope:** strict for the customer app (the S-04 lie was unlabelled and asserted «۶ رستوران فعال»). The panels' labelled sample fallback is MINOR (m-15), with the same target at P3, because staff running a live service need "we don't know", not labelled samples | `rezv-75` `d889a8e`: both panels PARTIAL, with real-row controls proven | yes |
| D-20 | **F001 design approved with amendments** (`rezv-1b`): (a) only a provider-accepted SMS or the guest's `/eta` counts as the warning. An unsent job does not (A4: null-as-success is impossible), so without a warning there is no auto no-show, a metric fires, and staff decide manually after the deadline. (b) the restaurant pays for `booking_late`, as for `booking_noshow`. (c) staff no-show is gated on `slotStart + grace (+ extension)`, otherwise 409. `/eta` is customer-kind only and reservation-owner only; the 10-minute floor is a named constant; migration 091 after 090 | premises checked on `c234479`: `notify.ts:18-22`, `lifecycle.ts:296`, `status/route.ts:41`, `reservations.js:165` | yes |
