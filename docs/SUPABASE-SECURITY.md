...
### RLS
- ✅ روی **۶۱ از ۷۳ جدولِ `public`** فعال است (`relrowsecurity = true`؛ شمارشِ زنده‌ی ۲۰۲۶-۰۹-۰۴؛ عددِ قبلیِ «۳۲ جدول» کهنه بود).
...
**نتیجه (اصلاحِ ۲۰۲۶-۰۹-۰۴ — P0-021):** این **یک** لایه است، نه دو.
`deny-by-default` فقط جلوی نقشی را می‌گیرد که owner **نباشد** — و تنها اتصالِ
موجود همان نقشِ owner است (`super=true bypassrls=true`)، پس RLS برای مسیرِ
واقعیِ داده صفر اثر دارد. با مهاجرت به Postgresِ self-hosted، anon key اصلاً
وجود نخواهد داشت، پس استدلالِ «محافظت در برابرِ anon» هم موضوعیت ندارد.
محافظتِ واقعی = اعتبارسنجیِ لایه‌ی اپلیکیشن (`requirePermission`،
`withRestaurantAuth`) که ماتریسِ ایزولاسیونِ A5 آن را می‌سنجد.
...
- [x] RLS روی ۶۱ از ۷۳ جدول فعال است — ⚠️ **این یک کنترلِ امنیتی نیست** (صفر policy،
      نقشِ owner، بدونِ FORCE). تیکِ این ردیف فقط یک واقعیتِ پیکربندی را ثبت می‌کند،
      نه یک محافظت. رجوع: `docs/SECURITY.md` §۱۰ و P0-021.
