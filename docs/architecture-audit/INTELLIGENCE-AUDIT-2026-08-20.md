...
> تست‌ها روی Postgres 16 + Redis 7 واقعی اجرا شدند: **۵۳۹/۵۳۹ سبز، ۰ شکست**
> (بعد از merge کردنِ `main` که رفعِ نشتِ زمانیِ PR #36 را آورد).
>
...
### ⚠️ تصحیحِ این گزارش — نشتِ زمانی (۲۰۲۶-۰۸-۲۰)

**نسخه‌ی اولِ همین سند این‌جا نوشته بود «بررسی شد و سالم است». آن حکم غلط بود.**

استدلالِ غلط این بود: `fetchTrainingRows` از
`ROWS BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING` استفاده می‌کرد و کامنتِ بالایِ همان
تابع ادعا می‌کرد «دقیقاً همان چیزی که در لحظه‌ی رزرو معلوم بوده». من آن کامنت را
پذیرفتم به‌جایِ اینکه خودم شرط را بسنجم — دقیقاً همان کاری که §۰ نقشه‌راه صریحاً منع
می‌کند («به کامنت‌ها اعتماد نکن»).

**نشتِ واقعی:** `ORDER BY created_at` می‌گوید کدام رزرو زودتر *ثبت* شده، ولی نتیجه‌ی یک
رزرو در لحظه‌ی *برگزاری* معلوم می‌شود. پس رزروی که زودتر ثبت شده ولی دیرتر برگزار
می‌شود، وضعیتِ `no_show`ِ آینده‌اش وارد ویژگیِ رزروهایِ بعدی می‌شد — مدل از آینده یاد
می‌گرفت.

**رفع:** در PR #36 (کامیتِ `e0b8157` روی `main`) با تبدیلِ window function به
`CROSS JOIN LATERAL` و شرطِ صریحِ نقطه-در-زمان `h.slot_start < r.created_at`، به‌همراهِ
`tests/temporal-leakage.integration.test.mts` (با کنترلِ مثبت و جهش‌آزمایی). این برنچ
آن رفع را merge کرده است.

**درسِ روشی:** نقص با دادهٔ معمولی بروز نمی‌کرد (۰ از ۱۳۷ رزروِ محیطِ توسعه فاصله‌ی
ثبت↔برگزاریِ بیش از یک روز داشت)، پس نه تست می‌شکست نه چشم می‌دید. تنها راهِ پیداکردنش
سنجشِ خودِ شرط بود، نه خواندنِ توضیحش.

split آموزش/هولدآوت زمانی است نه تصادفی — این بخش از اول درست بود.
...
مجموع: **۵۳۹/۵۳۹ سبز** روی Postgres+Redis واقعی (شاملِ ۳ تستِ نشتِ زمانیِ PR #36 که با merge وارد شد).
