...
- **۹۸ روت از ۱۳۵** از `parseBody`/`parseQuery`/`parseParams` استفاده می‌کنند
  (اندازه‌گیری‌شده ۲۰۲۶-۰۸-۲۴؛ عددِ قبلیِ «~۵۱» کهنه بود).
  ⚠️ نکته: `validate.ts` نسخه‌ی «قدیمی» نیست — همان شِیمِ Zod-مانند است که
  `schemas.ts` رویش ساخته شده و بازصادرش می‌کند. دو مسیرِ رقیب وجود ندارد.
- ⚠️ **تصحیحِ ۲۰۲۶-۰۸-۲۴ — ادعایِ قبلی غلط بود.** این خط می‌گفت
  «`zPhone` خروجی‌اش دقیقاً `+98XXXXXXXXXX` است … چون مستقیم برای DB lookup
  استفاده می‌شود». **`zPhone` هیچ نرمال‌سازی‌ای نمی‌کند** — فقط `trim` و
  بررسیِ *قالب* است و رشته را همان‌طور که آمده برمی‌گرداند.
  نرمال‌سازی کارِ `normalizePhone` در `lib/otp.ts` است (رقم‌ها را جدا می‌کند و
  به `+989XXXXXXXXX` می‌رساند، وگرنه خطا می‌دهد).
  چرا این خطِ غلط خطرناک بود: توسعه‌دهنده‌ای که باورش می‌کرد، خروجیِ `zPhone`
  را مستقیم برای lookup به‌کار می‌برد و `normalizePhone` را رد می‌کرد — نتیجه
  lookupِ ناموفق (`09123456789` در برابرِ `+989123456789`) یا ذخیره‌ی شماره‌ی
  نرمال‌نشده. دو لایه لازم است، نه یکی:
  **`zPhone` = قالب · `normalizePhone` = معنا.**
