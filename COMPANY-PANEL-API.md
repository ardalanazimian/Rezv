...
### `POST /api/v1/admin/restaurants` (جدید — ۲۰۲۶-۰۸-۲۶)
ساختِ **مستقیمِ** یک کسب‌وکار از پنلِ شرکت (حکمِ «پنلِ مادر»): تنانت + رستوران
(با میزهای شروع و اعتبارِ پیامکِ اولیه) + کارمندِ مالک، همه در **یک تراکنش**؛
و اختیاری، اعتبارنامه‌ی ورود با رمز برای مالک (همان سیاست‌های `lib/password`).

Body:
```json
{ "business_name": "...", "city": "?", "plan": "free|pro|enterprise?",
  "owner_phone": "09...", "owner_name": "?",
  "username": "?", "password": "?" }
```
- `username`/`password` یا **هر دو** یا هیچ‌کدام (نیمه‌پیکربندی رد می‌شود).
- شماره‌ای که از قبل حسابِ کسب‌وکار دارد صریح رد می‌شود (۴۲۲) — نه کسب‌وکارِ دوم.
- audit: `admin.business_created`. پاسخ: `201` با `restaurant/tenant_id/owner/login`.
- تفاوت با مسیرِ سفارش (`activateOrder`): این‌جا هیچ سفارشِ سایتی لازم نیست —
  onboardِ تلفنی/حضوری.
