...
export function cardHTML(r){
  ...
  return `<article class="rc reveal">
    ...
    <div class="rc-panel">
      <div class="rc-top"><div class="rc-name" style="display:flex;align-items:center;gap:6px;min-width:0"><span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(r.n)}</span>${r.slug?'':'<span class="demo-chip">نمونه</span>'}</div><div class="rc-rating">${Number.isFinite(r.rt)&&r.rt>0?`${icon('star',{size:14,fill:true,class:'star'})}${fmtFa(r.rt)}`:`<span style="opacity:.7">جدید</span>`}</div></div>
