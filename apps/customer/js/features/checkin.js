...
//  میز بنشیند. خودِ کدِ QR اعتبارنامه است — کسی که پشتِ میز نیست آن را
//  ندارد. `POST /api/v1/checkin` هم عمداً بدونِ احراز هویت است.
// ═══════════════════════════════════════════════════════════════════════
...
export async function runPendingCheckIn() {
  ...
  // `reservation_code === null` یعنی میز پیدا شد ولی رزروِ فعالی رویش نبود.
  ...
  resultSheet('✅', 'ورودت ثبت شد',
    ...
    `${tableLine}<div style="margin-top:6px;font-family:monospace;font-size:12px;color:var(--t3);direction:ltr">${esc(d.reservation_code)}</div>`);
}
