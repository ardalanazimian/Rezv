...
const RUNTIME = new Set([
  ...
  '--len',          // طولِ واژه‌ی کاشیِ اکسپلور (style خطی، ExploreHero) — با fallback
  // ۰۹-۱۳: --mx/--my، --kx-*، --progress، --reveal-delay و --vel با خودِ
  // نشانگر، کارتِ سه‌بعدی، نوارِ پیشرفت و Reveal ِ متحرک رفتند.
]);
...
// site.css هم متغیرِ محلی دارد (مثلاً --pw، عرضِ گوشیِ اسکرولِ زنده) — اعلانِ
// محلی همان‌قدر «تعریف‌شده» است که اعلانِ :root.
const defined = new Set([
  ...[...globals.matchAll(/(--[a-z0-9-]+)\s*:/g)].map((m) => m[1]),
  ...[...site.matchAll(/(--[a-z0-9-]+)\s*:/g)].map((m) => m[1]),
