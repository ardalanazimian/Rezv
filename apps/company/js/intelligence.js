...
function showAdminLoginCode(devCode, offline){
  setAdminGateLocked(true);
  // ⚠️ escapeِ صریح (۲۰۲۶-۰۸-۲۷) — همان رفعِ apps/business/js/staff-system.js:
  // `faD` فقط ارقام را نگاشت می‌کند و بقیه را دست‌نخورده رد می‌کند، و `devCode`
  // از پاسخِ سرور می‌آید. ترتیب عمدی است: اول faD بعد esc.
  document.getElementById('loginCard').innerHTML = `
    <div class="login-logo">${icon('mail',{size:34})}</div>
    <div class="login-title">کد ورود را وارد کنید</div>
    <div class="login-sub">کد ورود به شماره‌ی ${esc(faD(_adminPhone ?? ''))} ارسال شد</div>
    ...
    ${devCode ? `<div class="login-hint">${offline?'حالت دمو (بک‌اند متصل نیست):':'حالت توسعه:'} کد ورود <b>${esc(faD(devCode))}</b> است</div>` : ''}`;
