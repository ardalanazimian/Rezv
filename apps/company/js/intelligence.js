...
async function adminPasswordLogin(){
  ...
  // ⚠️ ۴۲۹ (DS-003 §۴‑۵) — حساس‌ترین موردِ کلِ سیستم روی همین مسیر است:
  // `adminTotpLogin` سقفِ **۵ در ۱۵ دقیقه** دارد (`ratelimit.ts:237`)،
  // بلندترین پنجره‌ی موجود. توستِ این پنل چند ثانیه‌ای است، پس برای قفلِ
  // ۱۵دقیقه‌ای به‌تنهایی بی‌فایده است — و بدتر: `reset()` دکمه را فوراً آزاد
  // می‌کند، پس مدیر دوباره می‌زند و **پنجره را تمدید می‌کند**.
  if (res.error?.code === 'RATE_LIMITED') {
    toast(coErrorIcon(res.error), coErrorText(res.error, '', fa));
    holdButtonForRetry(btn, res.error, 'ورود به پنل', fa);
    // ⚠️ `reset()` صدا زده نمی‌شود — همان چیزی که دکمه را آزاد می‌کرد.
    // ولی فیلدِ TOTP پاک می‌شود: کدِ مصرف‌شده دیگر معتبر نیست.
    const ti = document.getElementById('adminTotp');
    if (ti) ti.value = '';
    return;
  }

  // ⚠️ پیامِ سرور برای «کاربر نیست» و «رمز غلط» عمداً یکسان است — اینجا هم
  // دقیق‌تر نمی‌شود، وگرنه نشتی که سرور بست از سمتِ کلاینت باز می‌شود. پس
  // این مسیر از `coErrorText` **رد نمی‌شود**: متنِ سرور همان است که باید.
