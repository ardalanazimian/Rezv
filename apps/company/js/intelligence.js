  ...
  (async()=>{
    ...
    document.getElementById('v-security').innerHTML=`
      ...
      <div class="panel" style="margin-bottom:20px">
        ...
        ${eo.tier_distribution.length?`<div style="display:flex;gap:10px;flex-wrap:wrap">${eo.tier_distribution.map(t=>`<div style="flex:1;min-width:110px;padding:12px;border:1px solid var(--border);border-radius:10px;text-align:center"><div style="font-weight:800;font-size:15px">${esc(t.tier)}</div><div style="color:var(--t2);font-size:12px;margin-top:4px">${fa(t.count)} مشتری · اعتبارِ میانگین ${fa(t.avg_reliability)}</div></div>`).join('')}</div>`:'<div style="text-align:center;color:var(--t2);padding:20px">هنوز پروفایلی نیست</div>'}
      </div>
      <div class="panel" style="margin-bottom:20px">
        <div class="panel-head"><div><div class="panel-title">مشتریانِ فلگ‌دارِ سوءاستفاده</div><div class="panel-sub">می‌تونی فلگِ اشتباه رو پاک کنی یا دستی فلگ بزنی</div></div></div>
        ${d.flagged_abuse_users.length?d.flagged_abuse_users.map(u=>`
          <div style="display:flex;justify-content:space-between;align-items:center;gap:10px;padding:9px 0;border-bottom:1px solid var(--border);font-size:13px">
            <div>
        ...
        <div style="margin-top:14px;display:flex;gap:8px;flex-wrap:wrap">
          ...
          <button class="btn btn-sm" onclick="flagAbuseUi()">فلگِ دستی</button>
        </div>
      </div>
      <div class="panel" style="margin-bottom:20px">
...
async function clearAbuseFlagUi(userId){
  const res=await API.abuseFlagAction(userId,'clear');
  if(!res.ok){toast('','خطا: '+(res.error?.message||'نامشخص'));return;}
  toast('','فلگ پاک شد');
  rSecurity();
}
async function flagAbuseUi(){
  const el=document.getElementById('manualFlagUserId');
  const userId=(el?.value||'').trim();
  if(!userId){toast('','userId رو وارد کن');return;}
  const res=await API.abuseFlagAction(userId,'flag','فلگِ دستیِ ادمینِ پلتفرم از پنلِ امنیت');
  if(!res.ok){toast('','خطا: '+(res.error?.message||'نامشخص'));return;}
  toast('','کاربر فلگ شد');
  if(el) el.value='';
  rSecurity();
