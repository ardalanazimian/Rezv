...
async function activateSalesOrder(id) {
  ...
  // ⚠️ رفعِ blocker (۲۰۲۶-۰۹-۰۷): تأییدیه قبلاً هرگز نامِ مقصد را نشان نمی‌داد.
  //
  //   const who = o.suggested_tenant ? `«${name}»` : 'این کسب‌وکار';
  //
  // یعنی وقتی شناسه از همان `prompt`ِ بالا آمده بود، `suggested_tenant` طبقِ
  // تعریف null است، پس مدیر عبارتِ عمومیِ «این کسب‌وکار» را می‌دید و روی یک
  // UUIDِ تایپی/پیستیِ تأییدنشده تأیید می‌زد. یک رقمِ اشتباه = فعال‌شدنِ
  // اشتراکِ **پولی** روی رستورانِ غلط، بدونِ هیچ راهی برای تشخیص پیش از کلیک.
  //
  // رفع: شناسه پیش از تأیید به نام تبدیل می‌شود. منبعش همان RESTAURANTSِ
  // از‌قبل‌بارگذاری‌شده است (`api.js:127` هر ردیف `tenantId` دارد) — نه
  // درخواستِ تازه، نه endpointِ جدید.
  //
  // اگر پیدا نشد **مسدود نمی‌کنیم** — چون فهرست ممکن است صفحه‌بندی‌شده یا
  // ناقص باشد و آن‌وقت یک فعال‌سازیِ مشروع را می‌بستیم. به‌جایش نادانی را
  // صریح اعلام می‌کنیم و یک تأییدِ دومِ آگاهانه می‌گیریم. «نمی‌دانیم» به
  // کاربر گفته می‌شود، نه پنهان و نه به‌جای «درست است» جا زده می‌شود.
  const match = (typeof RESTAURANTS !== 'undefined' && Array.isArray(RESTAURANTS))
    ? RESTAURANTS.find((r) => r && String(r.tenantId) === String(tenantId))
    : null;

  let who;
  if (match) {
    who = `«${match.name}»`;
  } else if (o.suggested_tenant) {
    who = `«${o.suggested_tenant.tenantName}»`;
  } else {
    who = null; // نامعلوم — پایین‌تر جدا هشدار داده می‌شود
  }

  if (!who && !window.confirm(
    'این شناسه در فهرستِ رستوران‌های بارگذاری‌شده پیدا نشد:\n' +
    `${tenantId}\n\n` +
    'ممکن است درست باشد (فهرست شاید کامل نباشد)، ولی نمی‌توانیم نامِ مقصد را ' +
    'تأیید کنیم. با شناسه‌ی اشتباه، اشتراکِ پولی روی کسب‌وکارِ دیگری فعال می‌شود.\n\n' +
    'با این حال ادامه می‌دهی؟',
  )) return;

  if (!window.confirm(
    `اشتراکِ ${who || 'کسب‌وکارِ نامعلوم'} برای ${fa(o.months || 0)} ماه فعال شود؟\n` +
    `شناسه: ${tenantId}\n` +
