...
const PROV_REASON_FA = {
  ...
  // ⚠️ این ردیف جا افتاده بود (۲۰۲۶-۰۹-۰۹). سرور پنج `reason` می‌دهد و این
  // نگاشت چهارتا داشت — همان کلاسِ نگاشتِ ناقصی که امروز در `STATUS_META`،
  // `ST_FA` و نگاشتِ سطحِ وفاداری هم پیدا شد.
  //
  // fallback صادق بود (پیامِ سرور نشان داده می‌شد، نه «خطای ناشناخته»)، ولی
  // پیامِ سرور برای این یکی **مسیرِ خامِ HTTP** دارد:
  //     «… از POST /admin/restaurants/{id}/branches استفاده کنید»
  // یعنی مدیرِ پلتفرم یک اندپوینت را در UI می‌بیند — همان کلاسِ «از seated
  // به completed» در پنلِ رستوران.
  //
  // و متنِ درستِ انسانی از قبل **دو خط بالاتر** نوشته شده بود: ردیفِ
  // `duplicate_owner_phone` همین کار را با «افزودنِ شعبه» توضیح می‌دهد.
  attach_existing_owner_unsupported: 'این مالک از قبل رستوران دارد — برای شعبه‌ی جدید از صفحه‌ی همان رستوران «افزودنِ شعبه» را بزن.',
};
