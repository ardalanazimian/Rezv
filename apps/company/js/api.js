...
// ⚠️ `SUB_STATUS_LABEL` حذف شد (DS-003 §۴‑۲، ۲۰۲۶-۰۹-۰۹).
// اسپک آن را «دو نگاشتِ کاراکتربه‌کاراکتر یکسان» توصیف کرده بود؛ اندازه‌گیری
// دقیق‌تر است: این یکی **مرده** بود — تعریف می‌شد و **هیچ‌جا** خوانده نمی‌شد
// (`grep -rn SUB_STATUS_LABEL apps/ standalone/` → فقط دو خطِ تعریف).
// `STATUS_LABEL` در `data.js:24` تنها نسخه‌ی زنده است و پنج مصرف‌کننده دارد
// (`intelligence.js:68,136,216` · `overview.js:128` · `restaurant.js:5`).
//
// پس خطر واگراییِ رفتاری نبود؛ خطر این بود که کسی نسخه‌ی مرده را ویرایش کند
// و فکر کند کاری کرده — که بدترین شکلِ رونوشتِ دوم است، چون بی‌صدا بی‌اثر است.
