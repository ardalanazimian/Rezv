...
// کلیدهای دسترسی — هم‌راستا با schema بک‌اند (/restaurant/staff PATCH)، منهای
// کلیدهایی که هیچ روتی اجرایشان نمی‌کند.
const PERM_DEFS=[
  ...
  // ⚠️ `canManageStaff` عمداً اینجا **نیست**: هیچ روتی آن را اجرا نمی‌کند و
  // مدیریتِ کارکنان با **نقش** گیت می‌شود، نه با مجوز
  // (api/src/app/api/v1/restaurant/staff/route.ts:54-77 — تفویضش درِ
  // خودارتقایی باز می‌کرد؛ routing.js هم تب را با VIEW_ROLE گیت می‌کند).
  // پس این سوییچ فقط یک وعده‌ی توخالی به مدیر بود. اجرایِ سمتِ سرور اضافه
  // نمی‌شود — این تصمیم ثبت و در rbac-permission-coverage.test.mts قفل شده.
