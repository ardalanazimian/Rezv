...
async function doSendCampaign(){
  ...
  if(API.getToken()){
    ...
    if(aud.kind==='birthday'){
      // ⚠️ ۲۰۲۶-۰۹-۱۳: نتیجه‌ی loadClubMembers پیش‌تر دور ریخته می‌شد (CLUB فقط در
      // loyalty/reservations پر می‌شود) ⇒ «شماره‌ی معتبری نیست» با وجودِ عضو.
      if(!CLUB.length && typeof loadClubMembers==='function'){ CLUB=await loadClubMembers(); }
