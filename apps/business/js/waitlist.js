...
let _wlLoaded=false;
// ⚠️ رفعِ دادهٔ جعلی: `_wlLoaded` فقط وضعیتِ فراخوانیِ **صف** را می‌گفت، پس اگر
// فقط فراخوانیِ آمار می‌افتاد، اعدادِ دموی WL_DEMO_ANALYTICS (۱۴۲ ورودی،
// تبدیلِ ۶۹٪، ۲۲ دقیقه) به‌عنوانِ آمارِ واقعیِ رستوران رندر می‌شدند و نوتِ
// «نمونه» هم — چون صف واقعاً آمده بود — ناپدید می‌شد. حالا آمار پرچمِ خودش
// را دارد و بدونِ آن، کارت‌ها «—» می‌شوند نه عددِ ساختگی.
let _wlAnalyticsLoaded=false;
...
async function loadWaitlist(){
  ...
  const okAnalytics = a.ok && !!a.data;
  if(okAnalytics) WL_ANALYTICS=a.data;
  _wlLoaded=okQueue;
  _wlAnalyticsLoaded=okAnalytics;
  return okQueue;
}
...
function rWaitlist(){
  ...
  // آمار فقط وقتی عدد می‌شود که واقعاً از سرور آمده باشد؛ وگرنه «—».
  const wlNum=(v,suffix)=>_wlAnalyticsLoaded?fa(v)+(suffix||''):'—';
  ...
  document.getElementById('v-waitlist').innerHTML=wlDemoNote+`
    ...
    <div class="wl-stats-grid">
      ...
      <div class="wl-stat"><div class="wl-stat-ic" style="background:#FEF3C7">${icon('clock',{size:16})}</div><div><div class="wl-stat-num">${wlNum(A.avg_wait_minutes,'<span style="font-size:13px"> دق</span>')}</div><div class="wl-stat-lbl">میانگین انتظار</div></div></div>
