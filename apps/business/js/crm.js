...
async function custRenderCampaign(){
  // ⚠️ رفع‌شده (ممیزیِ ۲۰۲۶-۰۸-۲۵؛ حفظ‌شده در ادغام با خطِ ممیزیِ #68):
  // کارت‌های سگمنت رفتاری بودند (در خطر ریزش/VIP/جدید/تولد) ولی endpointِ SMS
  // فقط بر اساسِ tierِ باشگاه (gold/silver/bronze) یا حذفِ segment («همه»)
  // فیلتر می‌کند — پس ۳ از ۴ گزینه ۴۲۲ می‌گرفتند یا (بدتر، مثلِ «تولد») به
  // «همه» ارسال می‌شدند: مخاطبِ اشتباه. زنده تأیید شد: at_risk → HTTP 422.
  // حالا کارت‌ها دقیقاً همان چیزی‌اند که بک‌اند می‌تواند هدف بگیرد و شمارش از
  // اعضای واقعیِ باشگاه می‌آید. هدف‌گیریِ رفتاری (at_risk/تولد) نیازمندِ تغییرِ
  // بک‌اند است و در KNOWN_LIMITATIONS ثبت شد. (currentMonthFa از #68 همچنان
  // برای «تولد این ماه» در صفحه‌ی باشگاه استفاده می‌شود.)
