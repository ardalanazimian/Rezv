...
async function openCustomerDetail(userId){
  ...
  // ⚠️ `ST_FA` حذف شد (DS-003 §۴‑۲، ۲۰۲۶-۰۹-۰۹). شش کلید از هجده مقدارِ
  // `RStatus` داشت، پس در تایم‌لاینِ پروفایلِ مشتری وضعیت‌هایی مثل
  // `waitlisted`/`preparing`/`checked_in`/`running_late`/`dining`/`rejected`/
  // `expired`/`auto_cancelled`/`auto_confirmed` کلیدِ خامِ انگلیسی می‌شدند —
  // همان کلاسِ نگاشتِ ناقصی که DS-001 پشتِ میزِ پذیرش گرفت.
  //
  // ⚠️ و دو برچسب **واگرا** بودند، یعنی یک وضعیت در دو صفحه‌ی همین پنل دو
  // نامِ فارسیِ متفاوت داشت:
  //     seated   → «نشسته»    اینجا  ·  «سر میز» در STATUS_META
  //     no_show  → «عدم‌حضور» اینجا  ·  «نیومد»  در STATUS_META
  // حالا `STATUS_META` (data.js:5) تنها منبع است — همان‌که فهرستِ رزروها،
  // Outbox و متنِ خطای تغییرِ وضعیت هم از آن می‌خوانند.
  const tl=(d.timeline||[]);
  openModal(`
    ...
    <div style="max-height:240px;overflow-y:auto;margin-top:8px">
      ${tl.length?tl.map(r=>`
        <div style="display:flex;justify-content:space-between;align-items:center;padding:10px 12px;border:1px solid var(--border);border-radius:12px;margin-bottom:8px">
          <div>
            <div style="font-weight:700;font-size:13px">${esc(STATUS_META[r.status]?.label||r.status)} · ${fa(r.party_size)} نفر</div>
